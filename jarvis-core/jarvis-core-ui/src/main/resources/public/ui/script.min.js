...
b.one("/api/"+e[0],d.id).customPUT(d).then(l,function(a){f(a)}):b.all("/api/"+e[0]).one(e[1],d.id).customPUT(d).then(l,function(a){f(a)})},post:function(e,d,k,f){var l=function(b){b=c.plain(b);a.debug("[POST]",e,b);void 0!=k&&k(b)};1==e.length?b.all("/api/"+e[0]).post(d).then(l,function(a){f(a)}):b.all("/api/"+e[0]).all(e[1]).post(d).then(l,function(a){f(a)})},task:function(e,d,k,f,l,m){var q=function(b){var d=c.plain(b);a.debug("[TASK]",b,e,d);void 0!=l&&l(d)};1==e.length?b.all("/api/"+e[0]).one(d).customPOST(f,
"",{task:k}).then(q,function(a){m(a)}):b.all("/api/"+e[0]).all(e[1]).one(d).customPOST(f,"",{task:k}).then(q,function(a){m(a)})}},f={findAll:function(e,d,f,n,l,m){var q=function(b){var m=[];_.forEach(b,function(a){m.push(c.plain(a))});a.debug("[FIND/L/"+f+"]",e,d,n,m);l(m)};1==d.length?1==e.length?b.one("/api/"+e[0],n).all(d[0]).getList({href:f}).then(q,function(a){m(a)}):b.all("/api/"+e[0]).one(e[1],n).all(d[0]).getList({href:f}).then(q,function(a){m(a)}):1==e.length?b.one("/api/"+e[0],n).all(d[0]).all(d[1]).getList({href:f}).then(q,
