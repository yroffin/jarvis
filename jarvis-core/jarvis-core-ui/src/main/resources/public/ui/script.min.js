...
angular.module("JarvisApp.services.generic",["JarvisApp.services.filter"]).factory("genericResourceService",["$log","Restangular","filterService","toastService",function(a,b,c,d){a.info("genericResourceService");var e={findAll:function(f,d,e){var g=function(b){var e=[];_.forEach(b,function(a){e.push(c.plain(a))});a.debug("[FIND]",f,e);d(e)};1==f.length?b.all(f[0]).getList().then(g,function(a){e(a)}):b.all(f[0]).all(f[1]).getList().then(g,function(a){e(a)})},get:function(f,d,e,g){var h=function(b){b=
c.plain(b);a.debug("[GET]",f,b);void 0!=e&&e(b)};1==f.length?b.one(f[0],d).get().then(h,function(a){g(a)}):b.all(f[0]).one(f[1],d).get().then(h,function(a){g(a)})},delete:function(f,d,e,g){var h=function(b){b=c.plain(b);a.debug("[DELETE]",f,b);void 0!=e&&e(b)};1==f.length?b.one(f[0],d).remove().then(h,function(a){g(a)}):b.all(f[0]).one(f[1],d).remove().then(h,function(a){g(a)})},put:function(f,d,e,g){var h=function(b){b=c.plain(b);a.debug("[PUT]",f,b);void 0!=e&&e(b)};1==f.length?b.one(f[0],d.id).customPUT(d).then(h,
function(a){g(a)}):b.all(f[0]).one(f[1],d.id).customPUT(d).then(h,function(a){g(a)})},post:function(f,d,e,g){var h=function(b){b=c.plain(b);a.debug("[POST]",f,b);void 0!=e&&e(b)};1==f.length?b.all(f[0]).post(d).then(h,function(a){g(a)}):b.all(f[0]).all(f[1]).post(d).then(h,function(a){g(a)})},task:function(f,d,e,g,h,l){var n=function(b){var d=c.plain(b);a.debug("[TASK]",b,f,d);void 0!=h&&h(d)};1==f.length?b.all(f[0]).one(d).customPOST(g,"",{task:e}).then(n,function(a){l(a)}):b.all(f[0]).all(f[1]).one(d).customPOST(g,
"",{task:e}).then(n,function(a){l(a)})}},g={findAll:function(f,d,e,g,h,l){var n=function(b){var l=[];_.forEach(b,function(a){l.push(c.plain(a))});a.debug("[FIND/L/"+e+"]",f,d,g,l);h(l)};1==d.length?1==f.length?b.one(f[0],g).all(d[0]).getList({href:e}).then(n,function(a){l(a)}):b.all(f[0]).one(f[1],g).all(d[0]).getList({href:e}).then(n,function(a){l(a)}):1==f.length?b.one(f[0],g).all(d[0]).all(d[1]).getList({href:e}).then(n,function(a){l(a)}):b.all(f[0]).one(f[1],g).all(d[0]).all(d[1]).getList({href:e}).then(n,
function(a){l(a)})},post:function(f,d,e,g,h,l,n,p){var r=function(b){b=c.plain(b);a.debug("[POST/L]",f,b);void 0!=n&&n(b)};void 0!=l&&(l.href=e);1==d.length?1==f.length?b.one(f[0],g).one(d[0],h).customPOST(l).then(r,function(a){p(a)}):b.all(f[0]).one(f[1],g).one(d[0],h).customPOST(l).then(r,function(a){p(a)}):1==f.length?b.one(f[0],g).all(d[0]).one(d[1],h).customPOST(l).then(r,function(a){p(a)}):b.all(f[0]).one(f[1],g).all(d[0]).one(d[1],h).customPOST(l).then(r,function(a){p(a)})},put:function(f,
d,e,g,h,l,n,p,r){var t=function(b){b=c.plain(b);a.debug("[PUT/L]",f,b);void 0!=p&&p(b)},s={},s=void 0===n?{}:n;s.href=e;1==d.length?1==f.length?b.one(f[0],g).one(d[0],h).one(l).customPUT(s).then(t,function(a){r(a)}):b.all(f[0]).one(f[1],g).one(d[0],h).one(l).customPUT(s).then(t,function(a){r(a)}):1==f.length?b.one(f[0],g).all(d[0]).one(d[1],h).one(l).customPUT(s).then(t,function(a){r(a)}):b.all(f[0]).one(f[1],g).all(d[0]).one(d[1],h).one(l).customPUT(s).then(t,function(a){r(a)})},delete:function(f,
d,e,g,h,l,n,p){var r=function(b){b=c.plain(b);a.debug("[DELETE/L]",f,b);void 0!=n&&n(b)};1==d.length?1==f.length?b.one(f[0],g).one(d[0],h).remove({instance:l,href:e}).then(r,function(a){p(a)}):b.all(f[0]).one(f[1],g).one(d[0],h).remove({instance:l,href:e}).then(r,function(a){p(a)}):1==f.length?b.one(f[0],g).all(d[0]).one(d[1],h).remove({instance:l,href:e}).then(r,function(a){p(a)}):b.all(f[0]).one(f[1],g).all(d[0]).one(d[1],h).remove({instance:l,href:e}).then(r,function(a){p(a)})}};return{crud:function(a){return{findAll:function(b,
...
"$mdDialog","toastService",function(a,b,c){return{pickers:{confirm:function(a,c,g,f){a=b.confirm().title(a).textContent(c).ariaLabel("Confirm").ok(g).cancel(f);return b.show(a)},nodes:function(a,c,g,f){b.show({controller:f,templateUrl:"js/partials/dialog/nodesDialog.tmpl.html",parent:angular.element(document.body),targetEvent:a,clickOutsideToClose:!0}).then(function(a){c(a)},function(){g()})},graph:function(a,c,g,f,k){b.show({locals:{anchor:c},bindToController:!0,controller:k,templateUrl:"js/partials/dialog/graphDialog.tmpl.html",
targetEvent:a,clickOutsideToClose:!0}).then(function(a){g(a)},function(){f()})}}}}]);"use strict";
angular.module("JarvisApp.services.scope",["JarvisApp.services.generic"]).factory("genericScopeService",["$log","$filter","genericResourceService","genericPickerService","toastService",function(a,b,c,d,e){return{scope:{resources:function(g,f,k,m,q,h){a.debug("inject default resources scope",m);k.create=function(a,b){c.scope.collections.new(m,f(),a,q,b)};k.new=function(a){c.scope.collections.new(m,f(),h,q,a)};k.crud=c.crud(m.split("/"));k.remove=function(c,e){d.pickers.confirm(b("translate")("action.drop",
{name:c.name,id:c.id}),"",b("translate")("yes"),b("translate")("no")).then(function(){a.debug("remove",c);var b=c.id;_.remove(f(),function(a){return a.id==b});k.crud.delete(c.id);e&&e(c)},function(){a.debug("abort")})};k.load=function(b){g([]);q.findAll(function(c){g(c);a.debug(m+"-ctrl",c);b&&b()},e.failure)}},resource:function(e,f,k,m,q,h,l){a.debug("inject default resource scope",f);e.remove=function(h){d.pickers.confirm(b("translate")("action.drop",{name:h.name,id:h.id}),"",b("translate")("yes"),
b("translate")("no")).then(function(){c.scope.entity.remove(function(){e.go(k)},f,h,m)},function(){a.debug("abort")})};e.save=function(a){c.scope.entity.save(f,a,m)};e.duplicate=function(a){c.scope.entity.duplicate(function(){e.go(k)},f,a,m)}},resourceLink:function(e,f,k,m,q,h,l,n,p){a.debug("inject link",h);f.add=function(b){a.debug("link::add",b);c.scope.link.add(n,b,l,h,e())};f.update=function(b){a.debug("link::update",b);c.scope.link.save(n,b,h)};f.drop=function(f){d.pickers.confirm(b("translate")("action.remove.link",
{name:f.name,id:f.id}),"",b("translate")("yes"),b("translate")("no")).then(function(){a.debug("link::drop",f);p&&p.remove?p.remove(f):c.scope.link.remove(n,f,h,e());p&&p.afterRemove&&p.afterRemove(f)},function(){a.debug("abort")})}}}}}]);"use strict";
...
angular.module("JarvisApp.services.connector",[]).factory("connectorResourceService",["genericResourceService",function(a){return{connector:a.crud(["connectors"])}}]);"use strict";angular.module("JarvisApp.services.event",[]).factory("eventResourceService",["genericResourceService",function(a){return{event:a.crud(["events"])}}]);"use strict";angular.module("JarvisApp.services.trigger",[]).factory("triggerResourceService",["genericResourceService",function(a){return{trigger:a.crud(["triggers"])}}]);
...
a.setFullRequestInterceptor(function(a,c,d,e,g,f,k){return{element:a,params:f,headers:g,httpConfig:k}});a.setResponseExtractor(function(a){angular.isObject(a)&&(a.originalElement=a);return a})}]).controller("JarvisAppCtrl",["$scope","$log","$store","$http","$mdDialog","$mdSidenav","$mdMedia","$location","$state","genericPickerService","toastService","iotResourceService","eventResourceService","configurationResourceService",function(a,b,c,d,e,g,f,k,m,q,h,l,n,p){b.info("JarvisAppCtrl");a.config={};
...
"lg")});a.$watch(function(){return f("gt-lg")},function(b){b&&(a.media="gt-lg")});a.$watch(function(){return f("xl")},function(b){b&&(a.media="xl")});a.$watch(function(){return f("print")},function(b){b&&(a.media="print")});hljs.initHighlightingOnLoad();a.defaultValue=function(a,b){return void 0===a?b:a};a.loadSettings=function(){p.configuration.findAll(function(b){if(b=_.find(b,"active"))a.config=b},h.failure)};a.loadSettings();a.saveSettings=function(){p.configuration.put(a.config,function(a){b.info("Updated",
a)},h.failure)};a.closeSettings=function(){e.hide()};a.settings=function(c){var d=f("sm")||f("xs");e.show({scope:a,preserveScope:!0,templateUrl:"js/partials/dialog/settingsDialog.tmpl.html",parent:angular.element(document.body),targetEvent:c,clickOutsideToClose:!0,fullscreen:d}).then(function(){b.warn("Saving settings")},function(){b.warn("Cancel settings save")})};a.markdown=new showdown.Converter({tables:"true",ghCodeBlocks:"true"});a.helper=function(c){b.debug("State",m.current.name);d.get("js/helps/fr/"+
m.current.name+".markdown").then(function(c){c=a.markdown.makeHtml(c.data);a.help={content:c.replace(/<table>/g,'\x3ctable class\x3d"table table-striped table-bordered table-hover"\x3e')};b.debug("Help",a.help);g("right").toggle()})};a.toggleSidenav=function(a){g(a).toggle()};a.location=function(a,b){g(a).toggle();k.path(b)};a.go=function(a,b){m.go(a,b)};a.emit=function(a,c){b.debug("JarvisAppCtrl::emit",a,c,a.trigger);n.event.post({trigger:a.trigger,timestamp:(new Date).toISOString(),fired:!0,number:c},
function(a){b.debug("JarvisAppCtrl::emit",a);h.info("event "+a.trigger+"#"+a.id+" emitted")},h.failure)};a.execute=function(c){l.iot.task(c.id,"execute",{},function(c){b.debug("JarvisAppCtrl::execute",c);a.renderdata=c},h.failure)};a.graph=function(a,c){return q.pickers.graph(a,c,function(a){b.debug("Go")},function(){b.debug("Abort")},"graphDialogCtrl")};b.info("JarvisAppCtrl configured")}]).controller("graphDialogCtrl",["$scope","$log","$mdDialog",function(a,b,c){b.info("graphDialogCtrl");a.hide=
...
a.answer=function(a){c.hide(a)};a.elementsPicker=[{name:"Triggers",selectable:!1,nodes:[]}];a.crud=d.crud(["triggers"]);a.crud.findAll(function(b){_.each(b,function(b){b.selectable=!0;a.elementsPicker[0].nodes.push(b)})},e.failure)}]).controller("pickBlockDialogCtrl",["$scope","$log","$mdDialog","genericResourceService","toastService",function(a,b,c,d,e){b.info("pickBlockDialogCtrl");a.hide=function(){c.hide()};a.cancel=function(){c.cancel()};a.answer=function(a){c.hide(a)};a.elementsPicker=[{name:"Blocks",
selectable:!1,nodes:[]}];a.crudBlock=d.crud(["blocks"]);a.crudBlock.findAll(function(b){_.each(b,function(b){b.selectable=!0;a.elementsPicker[0].nodes.push(b)})},e.failure)}]);"use strict";
...
b.debug("result base64 value:",a);g.$setViewValue(atob(a));"fileLoaded"in e&&c.$eval(e.fileLoaded)};f.onprogress=function(a){"fileProgress"in e&&c.$eval(e.fileProgress,{$total:a.total,$loaded:a.loaded})};f.onerror=function(){"fileError"in e&&c.$eval(e.fileError,{$error:f.error})};var k=e.fileSelect;d.bind("change",function(a){a=a.target.files[0];""===k||"text"===k?f.readAsText(a,"UTF-8"):"data"===k&&f.readAsDataURL(a)})}}});"use strict";
...
a.renderdata=c;a.output=angular.toJson(c,!0)},m.failure)};a.load=function(){a.activeTab=d.tab;k.scope.entity.get(d.id,function(b){a.iot=b},e.iot);a.plugins=[];k.scope.collections.findAll("plugins",d.id,a.plugins,e.plugins);a.iots=[];k.scope.collections.findAll("iots",d.id,a.iots,e.iots);a.triggers=[];k.scope.collections.findAll("triggers",d.id,a.triggers,e.triggers);b.info("iot-ctrl")}}]);"use strict";
...
angular.module("JarvisApp.ctrl.triggers",["JarvisApp.services"]).controller("triggersCtrl",["$scope","$log","genericScopeService","triggerResourceService",function(a,b,c,d){c.scope.resources(function(b){a.triggers=b},function(){return a.triggers},a,"triggers",d.trigger,{name:"trigger name",icon:"settings_remote"})}]).controller("triggerCtrl",["$scope","$log","$stateParams","genericResourceService","genericScopeService","triggerResourceService","toastService",function(a,b,c,d,e,g,f){e.scope.resource(a,
"trigger","triggers",g.trigger);a.load=function(){d.scope.entity.get(c.id,function(b){a.trigger=b},g.trigger);b.info("trigger-ctrl")}}]);"use strict";
...
"toastService",function(a,b,c,d,e,g,f,k,m,q){e.scope.resource(a,"script","plugins",f.scripts);a.links={commands:{}};e.scope.resourceLink(function(){return a.commands},a.links.commands,"script","plugins",f.scripts,f.commands,{order:"1",name:"noname",nature:"info",type:"json"},c.id);a.execute=function(c){void 0!=c&&void 0!=c.id&&""!=c.id&&f.scripts.task(c.id,"execute",a.rawTestData,function(d){b.debug("pluginScriptCtrl::execute",c,d);a.rawoutput=d;a.output=angular.toJson(d,!0)},q.failure)};a.render=
...
"toastService",function(a,b,c,d,e,g,f,k){g.scope.resource(a,"connector","connectors",f.connector);a.ping=function(b){f.connector.task(b.id,"ping",b,function(b){a.status=angular.toJson(b,!0)})};a.load=function(){e.scope.entity.get(c.id,function(b){a.connector=b},f.connector);b.info("connector-ctrl",a.connectors)}}]);"use strict";
...
function(a,b,c,d,e,g,f,k,m,q){f.scope.resource(a,"snapshot","snapshots",k.snapshot);a.restore=function(a){void 0!=a&&void 0!=a.id&&""!=a.id&&k.snapshot.task(a.id,"restore",{},function(c){b.debug("snapshotCtrl::restore",a,c)},q.failure)};a.download=function(a){void 0!=a&&void 0!=a.id&&""!=a.id&&k.snapshot.task(a.id,"download",{},function(c){var e="export-"+d("date")(new Date,"yyyyMMdd-HHmmss")+".json",f=document.createElement("a");document.body.appendChild(f);f.style="display: none";b.debug("snapshot",
a,c);c=new Blob([angular.toJson(c,!0)],{type:"application/text"});c=window.URL.createObjectURL(c);f.href=c;f.download=e;f.click()},q.failure)};a.upload=function(a){$("#"+a).trigger("click")};a.loaded=function(a,c){a.json=c.data;b.debug("loaded",a,c)};a.load=function(){a.snapshots=[];g.scope.entity.get(c.id,function(b){a.snapshot=b},k.snapshot);b.info("snapshot-ctrl",a.snapshots)}}]);"use strict";
...
k,m,q){f.scope.resource(a,"cron","crons",k.cron);a.load=function(){a.crons=[];g.scope.entity.get(c.id,function(b){a.cron=b},k.cron);b.info("cron-ctrl",a.crons)}}]);"use strict";
angular.module("JarvisApp.ctrl.home",["JarvisApp.services"]).controller("homeCtrl",["$scope","$store","$log","viewResourceService","iotResourceService","toastService",function(a,b,c,d,e,g){a.onSwipeLeft=function(){a.tabIndex>=a.views.length-1?a.tabIndex=a.views.length-1:a.tabIndex++};a.onSwipeRight=function(){0>=a.tabIndex?a.tabIndex=0:a.tabIndex--};a.selectTab=function(b,c){a.tabIndex=c};a.activeTab=function(b,c){return a.tabIndex==c};a.load=function(){a.store=b;a.views=[];a.tabIndex=-1;d.view.findAll(function(b){var k=
[];_.forEach(b,function(a){a.ishome&&(a.urlBackground="url('"+a.url+"')",k.push(a))});g.info(k.length+" view(s)");_.forEach(k,function(a){c.info("loading view",a);d.iots.findAll(a.id,function(b){a.iots=b;var d=_.after(a.iots.length,function(){c.debug("Linked iots to view",a.iots)});_.forEach(a.iots,function(a){e.iot.task(a.id,"render",{},function(b){a.render=b;d(a)},g.failure)})},g.failure)});a.views=k;0<a.views.length&&(a.tabIndex=0)},g.failure);c.info("views-ctrl")}}]).controller("helperCtrl",["$scope",
...
"$log","$stateParams","$mdDialog","genericResourceService","genericScopeService","configurationResourceService","toastService",function(a,b,c,d,e,g,f,k){g.scope.resource(a,"configuration","configurations",f.configuration);a.load=function(){e.scope.entity.get(c.id,function(b){a.config=b},f.configuration);b.info("configuration-ctrl",a.configurations)}}]);"use strict";
...
"pluginResourceService","toastService",function(a,b,c,d,e,g,f,k,m,q,h){e.scope.resource(a,"scenario","scenarios",k.scenario);a.links={blocks:{},triggers:{}};e.scope.resourceLink(function(){return a.blocks},a.links.blocks,"scenario","scenarios",k.scenario,k.blocks,{order:"1"},c.id);e.scope.resourceLink(function(){return a.triggers},a.links.triggers,"scenario","scenarios",k.scenario,k.triggers,{order:"1"},c.id);a.build=function(a){var c=[];_.each(a,function(a){b.debug("graph",a.stage);var d="";_.each(a.nodes,
...
approved:{fill:"#58C4A3","font-size":12,"yes-text":"APPROVED","no-text":"n/a"},rejected:{fill:"#C45879","font-size":12,"yes-text":"n/a","no-text":"REJECTED"}}})})};a.chart=function(c){void 0!=c&&void 0!=c.id&&""!=c.id&&k.scenario.task(c.id,"render",{},function(d){b.debug("[SCENARIO/render]",c,d);a.codes=a.build(d);a.render(a.codes)},h.failure)};a.execute=function(c){void 0!=c&&void 0!=c.id&&""!=c.id&&k.scenario.task(c.id,"execute",{},function(d){b.debug("[SCENARIO/execute]",c,d);a.console=d},h.failure)};
a.load=function(){a.activeTab=c.tab;a.plugin={};g.scope.entity.get(c.id,function(b){a.scenario=b;a.chart(b)},k.scenario);a.blocks=[];g.scope.collections.findAll("blocks",c.id,a.blocks,k.blocks,function(a){_.forEach(a,function(a){a.plugins={};a.plugins.if=[];a.plugins.then=[];a.plugins.else=[];m.plugins.if.findAll(a.id,function(b){a.plugins.if=b});m.plugins.then.findAll(a.id,function(b){a.plugins.then=b});m.plugins.else.findAll(a.id,function(b){a.plugins.else=b});a.blocks={};a.blocks.then=[];a.blocks.else=
[];m.blocks.then.findAll(a.id,function(b){a.blocks.then=b});m.blocks.else.findAll(a.id,function(b){a.blocks.else=b});b.debug("blocks.plugins",a.plugins);b.debug("blocks.blocks",a.blocks)})});a.triggers=[];g.scope.collections.findAll("triggers",c.id,a.triggers,k.triggers);b.debug("scenario-ctrl",a.scenario)}}]);