angular.module("JarvisApp","ngMaterial ngMdIcons ngSanitize restangular angular-websocket ui.router ui.router.router pascalprecht.translate ngCookies hljs ui.tree JarvisApp.websocket JarvisApp.config JarvisApp.theme JarvisApp.routes JarvisApp.services JarvisApp.services.filter JarvisApp.services.generic JarvisApp.services.scope JarvisApp.services.plugin JarvisApp.services.command JarvisApp.services.scenario JarvisApp.services.snapshot JarvisApp.services.cron JarvisApp.services.view JarvisApp.services.configuration JarvisApp.services.property JarvisApp.services.connector JarvisApp.services.device JarvisApp.services.event JarvisApp.services.trigger JarvisApp.services.block JarvisApp.directives JarvisApp.directives.widgets JarvisApp.ctrl.plugins JarvisApp.ctrl.commands JarvisApp.ctrl.devices JarvisApp.ctrl.events JarvisApp.ctrl.triggers JarvisApp.ctrl.snapshots JarvisApp.ctrl.crons JarvisApp.ctrl.views JarvisApp.ctrl.configurations JarvisApp.ctrl.properties JarvisApp.ctrl.connectors JarvisApp.ctrl.scenarios JarvisApp.ctrl.blocks JarvisApp.ctrl.home".split(" "));
...
"use strict";angular.module("JarvisApp.services.cron",[]).factory("cronResourceService",["genericResourceService",function(a){return{cron:a.crud(["crons"])}}]);"use strict";angular.module("JarvisApp.services.device",[]).factory("deviceResourceService",["genericResourceService",function(a){return{device:a.crud(["devices"]),plugins:a.links(["devices"],["plugins","scripts"]),devices:a.links(["devices"],["devices"]),triggers:a.links(["devices"],["triggers"])}}]);"use strict";
...
fired:!0,number:b},function(a){d.debug("JarvisAppCtrl::emit",a);m.info("event "+a.trigger+"#"+a.id+" emitted")},m.failure)};b.execute=function(a){q.device.task(a.id,"execute",{},function(a){d.debug("JarvisAppCtrl::execute",a);b.renderdata=a},m.failure)};b.graph=function(a,b){return l.pickers.graph(a,b,function(a){d.debug("Go")},function(){d.debug("Abort")},"graphDialogCtrl")};b.boot=function(){d.info("JarvisAppCtrl");b.config={};b.media=h("xs");b.$watch(function(){return h("xs")},function(a){a&&(b.media=
...
nodes:[]}];a.crudDevice=c.crud(["devices"]);a.crudDevice.findAll(function(b){_.each(b,function(b){b.selectable=!1;a.elementsPicker[0].nodes.push(b)})},e.failure);a.crudScript=c.crud(["plugins","scripts"]);a.crudScript.findAll(function(b){_.each(b,function(b){b.selectable=!0;a.elementsPicker[1].nodes.push(b)})},e.failure)}]).controller("pickDeviceDialogCtrl",["$scope","$log","$mdDialog","genericResourceService","toastService",function(a,b,d,c,e){b.info("pickDeviceDialogCtrl");a.hide=function(){d.hide()};a.cancel=
function(){d.cancel()};a.answer=function(a){d.hide(a)};a.elementsPicker=[{name:"Connected Objects",selectable:!1,nodes:[]},{name:"Plugin Scripts",selectable:!1,nodes:[]}];a.crudDevice=c.crud(["devices"]);a.crudDevice.findAll(function(b){_.each(b,function(b){b.selectable=!0;a.elementsPicker[0].nodes.push(b)})},e.failure);a.crudScript=c.crud(["plugins","scripts"]);a.crudScript.findAll(function(b){_.each(b,function(b){b.selectable=!1;a.elementsPicker[1].nodes.push(b)})},e.failure)}]).controller("pickCommandDialogCtrl",
...
angular.module("JarvisApp.ctrl.plugins",["JarvisApp.services"]).controller("pluginsCtrl",["$scope","$log","genericScopeService","pluginResourceService",function(a,b,d,c){d.scope.resources(function(b){a.plugins=b},function(){return a.plugins},a,"plugins/scripts",c.scripts,{name:"script name",icon:"list",type:"script"})}]).controller("pluginScriptCtrl",["$scope","$log","$stateParams","genericResourceService","genericScopeService","genericPickerService","pluginResourceService","deviceResourceService","commandResourceService",
...
d;a.views=[];a.tabIndex=-1;e.view.findAll(function(b){var d=[];_.forEach(b,function(a){a.ishome&&(a.urlBackground="url('"+a.url+"')",d.push(a))});f.info(d.length+" view(s)");_.forEach(d,function(a){c.info("loading view",a);e.devices.findAll(a.id,function(b){a.devices=b;var d=_.after(a.devices.length,function(){c.debug("Linked devices to view",a.devices)});_.forEach(a.devices,function(a){g.device.task(a.id,"render",{},function(b){a.render=b;d(a)},f.failure)})},f.failure)});a.views=d;0<a.views.length&&(a.tabIndex=0)},f.failure);
c.info("home-ctrl")}}]).controller("helperCtrl",["$scope","$store","$log","viewResourceService","deviceResourceService","toastService",function(a,b,d,c,e,g){}]);"use strict";
