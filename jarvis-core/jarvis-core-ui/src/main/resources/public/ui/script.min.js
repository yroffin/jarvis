...
["$rootScope","$scope","$log","$store","$notification","$http","$mdDialog","$mdSidenav","$mdMedia","$location","$window","$state","genericPickerService","toastService","deviceResourceService","eventResourceService","configurationResourceService","oauth2ResourceService",function(a,b,d,c,e,g,f,h,m,p,k,l,q,n,r,t,s,u){b.defaultValue=function(a,b){return void 0===a?b:a};b.loadSettings=function(){s.configuration.findAll(function(a){(a=_.find(a,"active"))?b.config=a:n.failure("No default settings")},n.failure)};
b.saveSettings=function(){s.configuration.put(b.config,function(a){d.info("Updated",a)},n.failure)};b.closeSettings=function(){f.hide()};b.settings=function(a){var c=m("sm")||m("xs");f.show({scope:b,preserveScope:!0,templateUrl:"js/partials/dialog/settingsDialog.tmpl.html",parent:angular.element(document.body),targetEvent:a,clickOutsideToClose:!0,fullscreen:c}).then(function(){d.warn("Saving settings")},function(){d.warn("Cancel settings save")})};b.markdown=new showdown.Converter({tables:"true",
ghCodeBlocks:"true"});b.helper=function(a){d.debug("State",l.current.name);g.get("js/helps/fr/"+l.current.name+".markdown").then(function(a){a=b.markdown.makeHtml(a.data);b.help={content:a.replace(/<table>/g,'\x3ctable class\x3d"table table-striped table-bordered table-hover"\x3e')};d.debug("Help",b.help);h("right").toggle()})};b.toggleSidenav=function(a){h(a).toggle()};b.location=function(a,b){h(a).toggle();p.path(b)};b.go=function(a,b){l.go(a,b)};b.emit=function(a,b){d.debug("JarvisAppCtrl::emit",
a,b,a.trigger);t.event.post({trigger:a.trigger,timestamp:(new Date).toISOString(),fired:!0,number:b},function(a){d.debug("JarvisAppCtrl::emit",a);n.info("event "+a.trigger+"#"+a.id+" emitted")},n.failure)};b.execute=function(a){r.device.task(a.id,"execute",{},function(a){d.debug("JarvisAppCtrl::execute",a);b.renderdata=a},n.failure)};b.graph=function(a,b){return q.pickers.graph(a,b,function(a){d.debug("Go")},function(){d.debug("Abort")},"graphDialogCtrl")};b.$watch(function(a,b,d){return c.collection},
function(a,d,c){b.store=a});b.boot=function(){b.config={};b.version={angular:angular.version,angularmd:window.ngMaterial};d.info("JarvisAppCtrl",b.version);b.media=m("xs");b.$watch(function(){return m("xs")},function(a){a&&(b.media="xs")});b.$watch(function(){return m("gt-xs")},function(a){a&&(b.media="gt-xs")});b.$watch(function(){return m("sm")},function(a){a&&(b.media="sm")});b.$watch(function(){return m("gt-sm")},function(a){a&&(b.media="gt-sm")});b.$watch(function(){return m("md")},function(a){a&&
(b.media="md")});b.$watch(function(){return m("gt-md")},function(a){a&&(b.media="gt-md")});b.$watch(function(){return m("lg")},function(a){a&&(b.media="lg")});b.$watch(function(){return m("gt-lg")},function(a){a&&(b.media="gt-lg")});b.$watch(function(){return m("xl")},function(a){a&&(b.media="xl")});b.$watch(function(){return m("print")},function(a){a&&(b.media="print")});hljs.initHighlightingOnLoad();b.loadSettings();d.info("JarvisAppCtrl configured")};u.connect(b)}]).controller("extractTokenCtrl",
["$scope","$log","$location","$rootScope","$state","oauth2ResourceService",function(a,b,d,c,e,g){b.warn("extractTokenCtrl",e);b=d.path().substr(1).split("\x26");c={};for(e=0;e<b.length;e++){var f=b[e].split("\x3d"),h=f[0];c[h]=f[1];"access_token"===h&&(g.connect(a,c.access_token),d.path("/home"))}}]).controller("oauth2DialogCtrl",["$scope","$window","$log","$mdDialog","oauth2ResourceService",function(a,b,d,c,e){a.hide=function(){c.hide()};a.cancel=function(){c.cancel()};a.answer=function(a){e.config({client:a.client,
oauth2_redirect_uri:b.location.href.split("#")[0]},function(a){d.info("JarvisAppCtrl configured with",a);b.location.href=a.url},function(a){d.warn("no oauth2 configuration",a)});c.hide()}}]).controller("graphDialogCtrl",["$scope","$log","$mdDialog",function(a,b,d){b.info("graphDialogCtrl");a.hide=function(){d.hide()};a.cancel=function(){d.cancel()};a.answer=function(a){d.hide(a)}}]).controller("pickPluginDialogCtrl",["$scope","$log","$mdDialog","genericResourceService","toastService",function(a,b,
d,c,e){b.info("pickPluginDialogCtrl");a.hide=function(){d.hide()};a.cancel=function(){d.cancel()};a.answer=function(a){d.hide(a)};a.elementsPicker=[{name:"Connected Objects",selectable:!1,nodes:[]},{name:"Plugin Scripts",selectable:!1,nodes:[]}];a.crudDevice=c.crud(["devices"]);a.crudDevice.findAll(function(b){_.each(b,function(b){b.selectable=!1;a.elementsPicker[0].nodes.push(b)})},e.failure);a.crudScript=c.crud(["plugins","scripts"]);a.crudScript.findAll(function(b){_.each(b,function(b){b.selectable=
!0;a.elementsPicker[1].nodes.push(b)})},e.failure)}]).controller("pickDeviceDialogCtrl",["$scope","$log","$mdDialog","genericResourceService","toastService",function(a,b,d,c,e){b.info("pickDeviceDialogCtrl");a.hide=function(){d.hide()};a.cancel=function(){d.cancel()};a.answer=function(a){d.hide(a)};a.elementsPicker=[{name:"Connected Objects",selectable:!1,nodes:[]},{name:"Plugin Scripts",selectable:!1,nodes:[]}];a.crudDevice=c.crud(["devices"]);a.crudDevice.findAll(function(b){_.each(b,function(b){b.selectable=
!0;a.elementsPicker[0].nodes.push(b)})},e.failure);a.crudScript=c.crud(["plugins","scripts"]);a.crudScript.findAll(function(b){_.each(b,function(b){b.selectable=!1;a.elementsPicker[1].nodes.push(b)})},e.failure)}]).controller("pickCommandDialogCtrl",["$scope","$log","$mdDialog","genericResourceService","toastService",function(a,b,d,c,e){b.info("pickCommandDialogCtrl");a.hide=function(){d.hide()};a.cancel=function(){d.cancel()};a.answer=function(a){d.hide(a)};a.elementsPicker=[{name:"Commands",selectable:!1,
nodes:[]}];a.crud=c.crud(["commands"]);a.crud.findAll(function(b){_.each(b,function(b){b.selectable=!0;a.elementsPicker[0].nodes.push(b)})},e.failure)}]).controller("pickTriggerDialogCtrl",["$scope","$log","$mdDialog","genericResourceService","toastService",function(a,b,d,c,e){b.info("pickTriggerDialogCtrl");a.hide=function(){d.hide()};a.cancel=function(){d.cancel()};a.answer=function(a){d.hide(a)};a.elementsPicker=[{name:"Triggers",selectable:!1,nodes:[]}];a.crud=c.crud(["triggers"]);a.crud.findAll(function(b){_.each(b,
function(b){b.selectable=!0;a.elementsPicker[0].nodes.push(b)})},e.failure)}]).controller("pickCronDialogCtrl",["$scope","$log","$mdDialog","genericResourceService","toastService",function(a,b,d,c,e){b.info("pickCronDialogCtrl");a.hide=function(){d.hide()};a.cancel=function(){d.cancel()};a.answer=function(a){d.hide(a)};a.elementsPicker=[{name:"Crontab objects",selectable:!1,nodes:[]}];a.crud=c.crud(["crons"]);a.crud.findAll(function(b){_.each(b,function(b){b.selectable=!0;a.elementsPicker[0].nodes.push(b)})},
e.failure)}]).controller("pickBlockDialogCtrl",["$scope","$log","$mdDialog","genericResourceService","toastService",function(a,b,d,c,e){b.info("pickBlockDialogCtrl");a.hide=function(){d.hide()};a.cancel=function(){d.cancel()};a.answer=function(a){d.hide(a)};a.elementsPicker=[{name:"Blocks",selectable:!1,nodes:[]}];a.crudBlock=c.crud(["blocks"]);a.crudBlock.findAll(function(b){_.each(b,function(b){b.selectable=!0;a.elementsPicker[0].nodes.push(b)})},e.failure)}]);"use strict";
...
angular.module("JarvisApp.routes",["JarvisApp.config"]).config(["$urlRouterProvider",function(a){a.otherwise("/home")}]).config(["$stateProvider",function(a){a.state("token_access",{url:"/access_token\x3d:accesToken",controller:"extractTokenCtrl",template:""}).state("home",{url:"/home",controller:"homeCtrl",templateUrl:"/ui/js/partials/home/page.html"}).state("navigator",{url:"/navigator",template:"\x3cdiv jarvis-widget-navigator\x3e\x3c/div\x3e"}).state("helper-devices",{url:"/helper-devices",controller:"helperCtrl",
templateUrl:"/ui/js/partials/helper/jarvis-commands.svg"}).state("helper-scenarii",{url:"/helper-scenarii",controller:"helperCtrl",templateUrl:"/ui/js/partials/helper/jarvis-scenarii.svg"}).state("events",{url:"/events",controller:"eventsCtrl",templateUrl:"/ui/js/partials/events/page.html"}).state("triggers",{url:"/triggers",controller:"triggersCtrl",templateUrl:"/ui/js/partials/triggers/page.html"}).state("triggers-by-id",{url:"/triggers/:id?tab",controller:"triggerCtrl",templateUrl:"/ui/js/partials/triggers/trigger/page.html"}).state("devices",
{url:"/devices",controller:"devicesCtrl",templateUrl:"/ui/js/partials/devices/page.html"}).state("devices-by-id",{url:"/devices/:id?tab",controller:"deviceCtrl",templateUrl:"/ui/js/partials/devices/device/page.html"}).state("plugins",{url:"/plugins",controller:"pluginsCtrl",templateUrl:"/ui/js/partials/plugins/page.html"}).state("plugins-by-id-script",{url:"/plugins/scripts/:id?tab",controller:"pluginScriptCtrl",templateUrl:"/ui/js/partials/plugins/script/page.html"}).state("commands",{url:"/commands",
controller:"commandsCtrl",templateUrl:"/ui/js/partials/commands/page.html"}).state("commands-by-id",{url:"/commands/:id",controller:"commandCtrl",templateUrl:"/ui/js/partials/commands/command/page.html"}).state("views",{url:"/views",controller:"viewsCtrl",templateUrl:"/ui/js/partials/views/page.html"}).state("views-by-id",{url:"/views/:id",controller:"viewCtrl",templateUrl:"/ui/js/partials/views/view/page.html"}).state("configurations",{url:"/configurations",controller:"configurationsCtrl",templateUrl:"/ui/js/partials/configurations/page.html"}).state("configurations-by-id",
{url:"/configurations/:id",controller:"configurationCtrl",templateUrl:"/ui/js/partials/configurations/configuration/page.html"}).state("properties",{url:"/properties",controller:"propertiesCtrl",templateUrl:"/ui/js/partials/properties/page.html"}).state("properties-by-id",{url:"/properties/:id",controller:"propertyCtrl",templateUrl:"/ui/js/partials/properties/property/page.html"}).state("connectors",{url:"/connectors",controller:"connectorsCtrl",templateUrl:"/ui/js/partials/connectors/page.html"}).state("connectors-by-id",
...
"$stateParams","$parse",function(a,b,d){return{restrict:"E",template:'\x3cdiv ng-include\x3d"getJarvisInlineTemplateUrl()"\x3e\x3c/div\x3e',link:function(b,e,g){a.debug("jarvis-inline-template id",d(g.id)(b));b.getJarvisInlineTemplateUrl=function(){b.data=d(g.data)(b);return"/api/directives/html/devices/"+d(g.id)(b)}}}}]).directive("jarvisGauge",["$log","$store","$notification",function(a,b,d){return{restrict:"AC",controller:["$scope","$element","$attrs",function(c,e,d){a.debug("jarvis-gauge",d.id);
c.gauge=(new Gauge(document.getElementById(d.id))).setOptions({fontSize:10,lines:10,angle:0,lineWidth:.32,pointer:{length:.9,strokeWidth:.035,color:"#000000"},limitMax:"false",colorStart:"#6FADCF",colorStop:"#8FC0DA",strokeColor:"#E0E0E0",generateGradient:!0});c.gauge.maxValue=100;c.gauge.animationSpeed=10;c.gauge.set(0);c.$watch(function(){return 100*b.get("SystemIndicator","1",0).systemCpuLoad},function(a,b,d){c.gauge.set(a)})}]}}]);"use strict";
