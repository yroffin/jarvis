...
b.show(b.simple().position(c.getToastPosition()).textContent(d).hideDelay(3E3).capsule(!0))}}}]);myAppServices.factory("clientResourceService",["$q","$window","$rootScope","Restangular",function(a,b,c,d){angular.injector(["ng"]).get("$log").info("clientResourceService",a);return{findAll:function(a,b){d.all("clients").getList().then(function(b){a(b)},function(a){b(a)})}}}]);
myAppServices.factory("crontabResourceService",["$q","$window","$rootScope","Restangular",function(a,b,c,d){angular.injector(["ng"]).get("$log").info("crontabResourceService",a);return{findAll:function(a,b){d.all("crontabs").getList().then(function(b){a(b)},function(a){b(a)})}}}]);
myAppServices.factory("oauth2ResourceService",["$rootScope","$window","Restangular",function(a,b,c){angular.injector(["ng"]).get("$log");return{me:function(b,e){c.setDefaultHeaders({JarvisAuthToken:a.accessToken});c.one("/api/profile/me").get().then(function(a){b(a)},function(a){e(a)})},config:function(a,b,g){c.one("oauth2").get(a).then(function(a){b(a)},function(a){g(a)})}}}]);"use strict";myAppServices=angular.module("JarvisApp.services.filter",[]);
...
angular.module("JarvisApp.services.generic",["JarvisApp.services.filter"]).factory("genericResourceService",["$log","Restangular","filterService","toastService",function(a,b,c,d){a.info("genericResourceService");var e={findAll:function(f,d,e){var g=function(b){var e=[];_.forEach(b,function(a){e.push(c.plain(a))});a.debug("[FIND]",f,e);d(e)};1==f.length?b.all("/api/"+f[0]).getList().then(g,function(a){e(a)}):b.all("/api/"+f[0]).all(f[1]).getList().then(g,function(a){e(a)})},get:function(f,d,e,g){var k=
function(b){b=c.plain(b);a.debug("[GET]",f,b);void 0!=e&&e(b)};1==f.length?b.one("/api/"+f[0],d).get().then(k,function(a){g(a)}):b.all("/api/"+f[0]).one(f[1],d).get().then(k,function(a){g(a)})},delete:function(f,d,e,g){var k=function(b){b=c.plain(b);a.debug("[DELETE]",f,b);void 0!=e&&e(b)};1==f.length?b.one("/api/"+f[0],d).remove().then(k,function(a){g(a)}):b.all("/api/"+f[0]).one(f[1],d).remove().then(k,function(a){g(a)})},put:function(f,d,e,g){var k=function(b){b=c.plain(b);a.debug("[PUT]",f,b);
void 0!=e&&e(b)};1==f.length?b.one("/api/"+f[0],d.id).customPUT(d).then(k,function(a){g(a)}):b.all("/api/"+f[0]).one(f[1],d.id).customPUT(d).then(k,function(a){g(a)})},post:function(f,d,e,g){var k=function(b){b=c.plain(b);a.debug("[POST]",f,b);void 0!=e&&e(b)};1==f.length?b.all("/api/"+f[0]).post(d).then(k,function(a){g(a)}):b.all("/api/"+f[0]).all(f[1]).post(d).then(k,function(a){g(a)})},task:function(f,d,e,g,k,l){var m=function(b){var d=c.plain(b);a.debug("[TASK]",b,f,d);void 0!=k&&k(d)};1==f.length?
b.all("/api/"+f[0]).one(d).customPOST(g,"",{task:e}).then(m,function(a){l(a)}):b.all("/api/"+f[0]).all(f[1]).one(d).customPOST(g,"",{task:e}).then(m,function(a){l(a)})}},g={findAll:function(f,d,e,g,k,l){var m=function(b){var l=[];_.forEach(b,function(a){l.push(c.plain(a))});a.debug("[FIND/L/"+e+"]",f,d,g,l);k(l)};1==d.length?1==f.length?b.one("/api/"+f[0],g).all(d[0]).getList({href:e}).then(m,function(a){l(a)}):b.all("/api/"+f[0]).one(f[1],g).all(d[0]).getList({href:e}).then(m,function(a){l(a)}):
1==f.length?b.one("/api/"+f[0],g).all(d[0]).all(d[1]).getList({href:e}).then(m,function(a){l(a)}):b.all("/api/"+f[0]).one(f[1],g).all(d[0]).all(d[1]).getList({href:e}).then(m,function(a){l(a)})},post:function(f,d,e,g,k,l,m,p){var r=function(b){b=c.plain(b);a.debug("[POST/L]",f,b);void 0!=m&&m(b)};void 0!=l&&(l.href=e);1==d.length?1==f.length?b.one("/api/"+f[0],g).one(d[0],k).customPOST(l).then(r,function(a){p(a)}):b.all("/api/"+f[0]).one(f[1],g).one(d[0],k).customPOST(l).then(r,function(a){p(a)}):
1==f.length?b.one("/api/"+f[0],g).all(d[0]).one(d[1],k).customPOST(l).then(r,function(a){p(a)}):b.all("/api/"+f[0]).one(f[1],g).all(d[0]).one(d[1],k).customPOST(l).then(r,function(a){p(a)})},put:function(f,d,e,g,k,l,m,p,r){var t=function(b){b=c.plain(b);a.debug("[PUT/L]",f,b);void 0!=p&&p(b)},s={},s=void 0===m?{}:m;s.href=e;1==d.length?1==f.length?b.one("/api/"+f[0],g).one(d[0],k).one(l).customPUT(s).then(t,function(a){r(a)}):b.all("/api/"+f[0]).one(f[1],g).one(d[0],k).one(l).customPUT(s).then(t,
function(a){r(a)}):1==f.length?b.one("/api/"+f[0],g).all(d[0]).one(d[1],k).one(l).customPUT(s).then(t,function(a){r(a)}):b.all("/api/"+f[0]).one(f[1],g).all(d[0]).one(d[1],k).one(l).customPUT(s).then(t,function(a){r(a)})},delete:function(f,d,e,g,k,l,m,p){var r=function(b){b=c.plain(b);a.debug("[DELETE/L]",f,b);void 0!=m&&m(b)};1==d.length?1==f.length?b.one("/api/"+f[0],g).one(d[0],k).remove({instance:l,href:e}).then(r,function(a){p(a)}):b.all("/api/"+f[0]).one(f[1],g).one(d[0],k).remove({instance:l,
href:e}).then(r,function(a){p(a)}):1==f.length?b.one("/api/"+f[0],g).all(d[0]).one(d[1],k).remove({instance:l,href:e}).then(r,function(a){p(a)}):b.all("/api/"+f[0]).one(f[1],g).all(d[0]).one(d[1],k).remove({instance:l,href:e}).then(r,function(a){p(a)})}};return{crud:function(a){return{findAll:function(b,d){return e.findAll(a,b,d)},get:function(b,d,c){return e.get(a,b,d,c)},delete:function(b,d,c){return e.delete(a,b,d,c)},put:function(b,d,c){return e.put(a,b,d,c)},post:function(b,d,c){return e.post(a,
b,d,c)},task:function(b,d,c,g,l){return e.task(a,b,d,c,g,l)}}},links:function(a,b,d){void 0===d&&(d="HREF");return{findAll:function(c,e,l){return g.findAll(a,b,d,c,e,l)},delete:function(c,e,l,m,p){return g.delete(a,b,d,c,e,l,m,p)},put:function(c,e,l,m,p,r){return g.put(a,b,d,c,e,l,m,p,r)},post:function(c,e,l,m,p){return g.post(a,b,d,c,e,l,m,p)}}},scope:{collections:{new:function(b,c,e,g,k){g.post(e,function(d){a.debug(b+" "+d.name+"#"+d.id+" created");c.push(d);k&&k(d)},d.failure)},findAll:function(b,
...
function(e){d.info(b+" "+c.name+"#"+c.id+" duplicated");a()},d.failure)}},link:{add:function(b,c,e,g,k){void 0!=c&&void 0!=c.id&&""!=c.id&&g.post(b,c.id,e,function(b){a.debug("addLink",b);k.push(b)},d.failure)},save:function(b,c,e){void 0!=c&&void 0!=c.id&&""!=c.id&&e.put(b,c.id,c.instance,c.extended,function(b){a.debug("updateLink - before",c);a.debug("updateLink - after",b)},d.failure)},remove:function(b,c,e,g){void 0!=c&&void 0!=c.id&&""!=c.id&&e.delete(b,c.id,c.instance,function(b){var d=c.instance;
_.remove(g,function(a){return a.instance==d});a.debug("removeLink - before",c)},d.failure)}},combo:{findAll:function(b,c,e){a.debug("loading for select ",b,c);e.findAll(function(a){_.forEach(a,function(a){c.push({id:a.id,name:a.name})})},d.failure)}}}}}]).factory("genericPickerService",["$log","$mdDialog","toastService",function(a,b,c){return{pickers:{confirm:function(a,c,g,f){a=b.confirm().title(a).textContent(c).ariaLabel("Confirm").ok(g).cancel(f);return b.show(a)},nodes:function(a,c,g,f){b.show({controller:f,
templateUrl:"js/partials/dialog/nodesDialog.tmpl.html",parent:angular.element(document.body),targetEvent:a,clickOutsideToClose:!0}).then(function(a){c(a)},function(){g()})},graph:function(a,c,g,f,h){b.show({locals:{anchor:c},bindToController:!0,controller:h,templateUrl:"js/partials/dialog/graphDialog.tmpl.html",targetEvent:a,clickOutsideToClose:!0}).then(function(a){g(a)},function(){f()})}}}}]);"use strict";
angular.module("JarvisApp.services.scope",["JarvisApp.services.generic"]).factory("genericScopeService",["$log","$filter","genericResourceService","genericPickerService","toastService",function(a,b,c,d,e){return{scope:{resources:function(g,f,h,n,q,k){a.debug("inject default resources scope",n);h.create=function(a,b){c.scope.collections.new(n,f(),a,q,b)};h.new=function(a){c.scope.collections.new(n,f(),k,q,a)};h.crud=c.crud(n.split("/"));h.remove=function(c,e){d.pickers.confirm(b("translate")("action.drop",
{name:c.name,id:c.id}),"",b("translate")("yes"),b("translate")("no")).then(function(){a.debug("remove",c);var b=c.id;_.remove(f(),function(a){return a.id==b});h.crud.delete(c.id);e&&e(c)},function(){a.debug("abort")})};h.load=function(b){g([]);q.findAll(function(c){g(c);a.debug(n+"-ctrl",c);b&&b()},e.failure)}},resource:function(e,f,h,n,q,k,l){a.debug("inject default resource scope",f);e.remove=function(k){d.pickers.confirm(b("translate")("action.drop",{name:k.name,id:k.id}),"",b("translate")("yes"),
b("translate")("no")).then(function(){c.scope.entity.remove(function(){e.go(h)},f,k,n)},function(){a.debug("abort")})};e.save=function(a){c.scope.entity.save(f,a,n)};e.duplicate=function(a){c.scope.entity.duplicate(function(){e.go(h)},f,a,n)}},resourceLink:function(e,f,h,n,q,k,l,m,p){a.debug("inject link",k);f.add=function(b){a.debug("link::add",b);c.scope.link.add(m,b,l,k,e())};f.update=function(b){a.debug("link::update",b);c.scope.link.save(m,b,k)};f.drop=function(f){d.pickers.confirm(b("translate")("action.remove.link",
{name:f.name,id:f.id}),"",b("translate")("yes"),b("translate")("no")).then(function(){a.debug("link::drop",f);p&&p.remove?p.remove(f):c.scope.link.remove(m,f,k,e());p&&p.afterRemove&&p.afterRemove(f)},function(){a.debug("abort")})}}}}}]);"use strict";
angular.module("JarvisApp.services.plugin",[]).factory("pluginResourceService",["$log","Restangular","filterService","commandResourceService","genericResourceService",function(a,b,c,d,e){return{plugins:{findAll:function(a,d){var e=[],n=["scripts"],q=_.after(n.length,function(b){a(b)});_.forEach(n,function(a){b.all("plugins").all(a).getList().then(function(a){_.forEach(a,function(a){e.push(c.plain(a))});q(e)},function(a){d(a)})})}},scripts:e.crud(["plugins","scripts"]),commands:e.links(["plugins",
...
"use strict";
angular.module("JarvisApp.config",[]).config(["$mdIconProvider",function(a){angular.injector(["ng"]).get("$log").info("$mdIconProvider",a)}]).config(["$translateProvider",function(a){angular.injector(["ng"]).get("$log").info("$translateProvider",a);a.useStaticFilesLoader({prefix:"js/l10n/",suffix:".json"});a.preferredLanguage("fr_FR");a.useSanitizeValueStrategy(null)}]).config(["RestangularProvider",function(a){a.setDefaultHeaders({"content-type":"application/json"});a.setFullRequestInterceptor(function(a,c,
d,e,g,f,h){return{element:a,params:f,headers:g,httpConfig:h}});a.setResponseExtractor(function(a){angular.isObject(a)&&(a.originalElement=a);return a})}]).controller("JarvisAppCtrl",["$rootScope","$scope","$log","$store","$http","$mdDialog","$mdSidenav","$mdMedia","$location","$window","$state","genericPickerService","toastService","iotResourceService","eventResourceService","configurationResourceService","oauth2ResourceService",function(a,b,c,d,e,g,f,h,n,q,k,l,m,p,r,t,s){b.defaultValue=function(a,
b){return void 0===a?b:a};b.loadSettings=function(){t.configuration.findAll(function(a){if(a=_.find(a,"active"))b.config=a},m.failure)};b.saveSettings=function(){t.configuration.put(b.config,function(a){c.info("Updated",a)},m.failure)};b.closeSettings=function(){g.hide()};b.settings=function(a){var d=h("sm")||h("xs");g.show({scope:b,preserveScope:!0,templateUrl:"js/partials/dialog/settingsDialog.tmpl.html",parent:angular.element(document.body),targetEvent:a,clickOutsideToClose:!0,fullscreen:d}).then(function(){c.warn("Saving settings")},
function(){c.warn("Cancel settings save")})};b.markdown=new showdown.Converter({tables:"true",ghCodeBlocks:"true"});b.helper=function(a){c.debug("State",k.current.name);e.get("js/helps/fr/"+k.current.name+".markdown").then(function(a){a=b.markdown.makeHtml(a.data);b.help={content:a.replace(/<table>/g,'\x3ctable class\x3d"table table-striped table-bordered table-hover"\x3e')};c.debug("Help",b.help);f("right").toggle()})};b.toggleSidenav=function(a){f(a).toggle()};b.location=function(a,b){f(a).toggle();
n.path(b)};b.go=function(a,b){k.go(a,b)};b.emit=function(a,b){c.debug("JarvisAppCtrl::emit",a,b,a.trigger);r.event.post({trigger:a.trigger,timestamp:(new Date).toISOString(),fired:!0,number:b},function(a){c.debug("JarvisAppCtrl::emit",a);m.info("event "+a.trigger+"#"+a.id+" emitted")},m.failure)};b.execute=function(a){p.iot.task(a.id,"execute",{},function(a){c.debug("JarvisAppCtrl::execute",a);b.renderdata=a},m.failure)};b.graph=function(a,b){return l.pickers.graph(a,b,function(a){c.debug("Go")},
function(){c.debug("Abort")},"graphDialogCtrl")};b.boot=function(){c.info("JarvisAppCtrl");b.config={};b.media=h("xs");b.$watch(function(){return h("xs")},function(a){a&&(b.media="xs")});b.$watch(function(){return h("gt-xs")},function(a){a&&(b.media="gt-xs")});b.$watch(function(){return h("sm")},function(a){a&&(b.media="sm")});b.$watch(function(){return h("gt-sm")},function(a){a&&(b.media="gt-sm")});b.$watch(function(){return h("md")},function(a){a&&(b.media="md")});b.$watch(function(){return h("gt-md")},
function(a){a&&(b.media="gt-md")});b.$watch(function(){return h("lg")},function(a){a&&(b.media="lg")});b.$watch(function(){return h("gt-lg")},function(a){a&&(b.media="gt-lg")});b.$watch(function(){return h("xl")},function(a){a&&(b.media="xl")});b.$watch(function(){return h("print")},function(a){a&&(b.media="print")});hljs.initHighlightingOnLoad();b.loadSettings();c.info("JarvisAppCtrl configured")};b.login=function(){g.show({controller:"oauth2DialogCtrl",templateUrl:"/ui/js/partials/dialog/oauth2Dialog.tmpl.html",
parent:angular.element(document.body),clickOutsideToClose:!0,fullscreen:!1})};b.checkProfile=function(){c.info("Profile checking ",a.accessToken);void 0===a.accessToken?(c.warn("no token"),b.login()):s.me(function(a){c.info("profile",a)},function(a){c.warn("no profile",a);b.login()})}}]).controller("extractTokenCtrl",["$scope","$log","$location","$rootScope","$state",function(a,b,c,d,e){b.warn("extractTokenCtrl",e);for(var g=c.path().substr(1).split("\x26"),f={},h=0;h<g.length;h++){var n=g[h].split("\x3d"),
q=n[0];f[q]=n[1];"access_token"===q&&(b.info("retrieve token",f),d.accessToken=f.access_token)}a.checkProfile();a.boot();c.path("/home");b.warn("extractTokenCtrl - done",e)}]).controller("oauth2DialogCtrl",["$scope","$window","$log","$mdDialog","oauth2ResourceService",function(a,b,c,d,e){a.hide=function(){d.hide()};a.cancel=function(){d.cancel()};a.answer=function(a){e.config({client:a.client,oauth2_redirect_uri:b.location.href.split("#")[0]},function(a){c.info("JarvisAppCtrl configured with",a);
b.location.href=a.url},function(a){c.warn("no oauth2 configuration",a)});d.hide()}}]).controller("graphDialogCtrl",["$scope","$log","$mdDialog",function(a,b,c){b.info("graphDialogCtrl");a.hide=function(){c.hide()};a.cancel=function(){c.cancel()};a.answer=function(a){c.hide(a)}}]).controller("pickPluginDialogCtrl",["$scope","$log","$mdDialog","genericResourceService","toastService",function(a,b,c,d,e){b.info("pickPluginDialogCtrl");a.hide=function(){c.hide()};a.cancel=function(){c.cancel()};a.answer=
function(a){c.hide(a)};a.elementsPicker=[{name:"Connected Objects",selectable:!1,nodes:[]},{name:"Plugin Scripts",selectable:!1,nodes:[]}];a.crudIot=d.crud(["iots"]);a.crudIot.findAll(function(b){_.each(b,function(b){b.selectable=!1;a.elementsPicker[0].nodes.push(b)})},e.failure);a.crudScript=d.crud(["plugins","scripts"]);a.crudScript.findAll(function(b){_.each(b,function(b){b.selectable=!0;a.elementsPicker[1].nodes.push(b)})},e.failure)}]).controller("pickIotDialogCtrl",["$scope","$log","$mdDialog",
"genericResourceService","toastService",function(a,b,c,d,e){b.info("pickIotDialogCtrl");a.hide=function(){c.hide()};a.cancel=function(){c.cancel()};a.answer=function(a){c.hide(a)};a.elementsPicker=[{name:"Connected Objects",selectable:!1,nodes:[]},{name:"Plugin Scripts",selectable:!1,nodes:[]}];a.crudIot=d.crud(["iots"]);a.crudIot.findAll(function(b){_.each(b,function(b){b.selectable=!0;a.elementsPicker[0].nodes.push(b)})},e.failure);a.crudScript=d.crud(["plugins","scripts"]);a.crudScript.findAll(function(b){_.each(b,
function(b){b.selectable=!1;a.elementsPicker[1].nodes.push(b)})},e.failure)}]).controller("pickCommandDialogCtrl",["$scope","$log","$mdDialog","genericResourceService","toastService",function(a,b,c,d,e){b.info("pickCommandDialogCtrl");a.hide=function(){c.hide()};a.cancel=function(){c.cancel()};a.answer=function(a){c.hide(a)};a.elementsPicker=[{name:"Commands",selectable:!1,nodes:[]}];a.crud=d.crud(["commands"]);a.crud.findAll(function(b){_.each(b,function(b){b.selectable=!0;a.elementsPicker[0].nodes.push(b)})},
e.failure)}]).controller("pickTriggerDialogCtrl",["$scope","$log","$mdDialog","genericResourceService","toastService",function(a,b,c,d,e){b.info("pickTriggerDialogCtrl");a.hide=function(){c.hide()};a.cancel=function(){c.cancel()};a.answer=function(a){c.hide(a)};a.elementsPicker=[{name:"Triggers",selectable:!1,nodes:[]}];a.crud=d.crud(["triggers"]);a.crud.findAll(function(b){_.each(b,function(b){b.selectable=!0;a.elementsPicker[0].nodes.push(b)})},e.failure)}]).controller("pickCronDialogCtrl",["$scope",
"$log","$mdDialog","genericResourceService","toastService",function(a,b,c,d,e){b.info("pickCronDialogCtrl");a.hide=function(){c.hide()};a.cancel=function(){c.cancel()};a.answer=function(a){c.hide(a)};a.elementsPicker=[{name:"Crontab objects",selectable:!1,nodes:[]}];a.crud=d.crud(["crons"]);a.crud.findAll(function(b){_.each(b,function(b){b.selectable=!0;a.elementsPicker[0].nodes.push(b)})},e.failure)}]).controller("pickBlockDialogCtrl",["$scope","$log","$mdDialog","genericResourceService","toastService",
function(a,b,c,d,e){b.info("pickBlockDialogCtrl");a.hide=function(){c.hide()};a.cancel=function(){c.cancel()};a.answer=function(a){c.hide(a)};a.elementsPicker=[{name:"Blocks",selectable:!1,nodes:[]}];a.crudBlock=d.crud(["blocks"]);a.crudBlock.findAll(function(b){_.each(b,function(b){b.selectable=!0;a.elementsPicker[0].nodes.push(b)})},e.failure)}]);"use strict";
...
a.renderdata=c;a.output=angular.toJson(c,!0)},n.failure)};a.load=function(){a.activeTab=d.tab;h.scope.entity.get(d.id,function(b){a.iot=b},e.iot);a.plugins=[];h.scope.collections.findAll("plugins",d.id,a.plugins,e.plugins);a.iots=[];h.scope.collections.findAll("iots",d.id,a.iots,e.iots);a.triggers=[];h.scope.collections.findAll("triggers",d.id,a.triggers,e.triggers);b.info("iot-ctrl")}}]);"use strict";
...
"toastService",function(a,b,c,d,e,g,f,h,n,q){e.scope.resource(a,"script","plugins",f.scripts);a.links={commands:{}};e.scope.resourceLink(function(){return a.commands},a.links.commands,"script","plugins",f.scripts,f.commands,{order:"1",name:"noname",nature:"json",type:"data"},c.id);a.execute=function(c){void 0!=c&&void 0!=c.id&&""!=c.id&&f.scripts.task(c.id,"execute",a.rawTestData,function(d){b.debug("pluginScriptCtrl::execute",c,d);a.rawoutput=d;a.output=angular.toJson(d,!0)},q.failure)};a.render=
...
h,n,q){f.scope.resource(a,"cron","crons",h.cron);a.toggle=function(a){b.info(a);h.cron.task(a.id,"toggle",{},function(a){q.info("crontab "+crontab.name+"#"+crontab.id+" toggled to "+crontab.status)},q.failure)};a.test=function(a){b.info(a);h.cron.task(a.id,"test",{},function(a){q.info("crontab "+crontab.name+"#"+crontab.id+" toggled to "+crontab.status)},q.failure)};a.load=function(){a.crons=[];g.scope.entity.get(c.id,function(b){a.cron=b},h.cron);b.info("cron-ctrl",a.crons)}}]);"use strict";
angular.module("JarvisApp.ctrl.home",["JarvisApp.services"]).controller("homeCtrl",["$scope","$rootScope","$store","$log","viewResourceService","iotResourceService","toastService","oauth2ResourceService",function(a,b,c,d,e,g,f,h){a.onSwipeLeft=function(){a.tabIndex>=a.views.length-1?a.tabIndex=a.views.length-1:a.tabIndex++};a.onSwipeRight=function(){0>=a.tabIndex?a.tabIndex=0:a.tabIndex--};a.selectTab=function(b,c){a.tabIndex=c};a.activeTab=function(b,c){return a.tabIndex==c};a.load=function(){a.checkProfile();
a.store=c;a.views=[];a.tabIndex=-1;e.view.findAll(function(b){var c=[];_.forEach(b,function(a){a.ishome&&(a.urlBackground="url('"+a.url+"')",c.push(a))});f.info(c.length+" view(s)");_.forEach(c,function(a){d.info("loading view",a);e.iots.findAll(a.id,function(b){a.iots=b;var c=_.after(a.iots.length,function(){d.debug("Linked iots to view",a.iots)});_.forEach(a.iots,function(a){g.iot.task(a.id,"render",{},function(b){a.render=b;c(a)},f.failure)})},f.failure)});a.views=c;0<a.views.length&&(a.tabIndex=
0)},f.failure);d.info("home-ctrl")}}]).controller("helperCtrl",["$scope","$store","$log","viewResourceService","iotResourceService","toastService",function(a,b,c,d,e,g){}]);"use strict";
...
"pluginResourceService","toastService",function(a,b,c,d,e,g,f,h,n,q,k){e.scope.resource(a,"scenario","scenarios",h.scenario);a.links={blocks:{},triggers:{}};e.scope.resourceLink(function(){return a.blocks},a.links.blocks,"scenario","scenarios",h.scenario,h.blocks,{order:"1"},c.id);e.scope.resourceLink(function(){return a.triggers},a.links.triggers,"scenario","scenarios",h.scenario,h.triggers,{order:"1"},c.id);a.build=function(a){var c=[];_.each(a,function(a){b.debug("graph",a.stage);var d="";_.each(a.nodes,
...
a.load=function(){a.activeTab=c.tab;a.plugin={};g.scope.entity.get(c.id,function(b){a.scenario=b;a.chart(b)},h.scenario);a.blocks=[];g.scope.collections.findAll("blocks",c.id,a.blocks,h.blocks,function(a){_.forEach(a,function(a){a.plugins={};a.plugins.if=[];a.plugins.then=[];a.plugins.else=[];n.plugins.if.findAll(a.id,function(b){a.plugins.if=b});n.plugins.then.findAll(a.id,function(b){a.plugins.then=b});n.plugins.else.findAll(a.id,function(b){a.plugins.else=b});a.blocks={};a.blocks.then=[];a.blocks.else=
[];n.blocks.then.findAll(a.id,function(b){a.blocks.then=b});n.blocks.else.findAll(a.id,function(b){a.blocks.else=b});b.debug("blocks.plugins",a.plugins);b.debug("blocks.blocks",a.blocks)})});a.triggers=[];g.scope.collections.findAll("triggers",c.id,a.triggers,h.triggers);b.debug("scenario-ctrl",a.scenario)}}]);