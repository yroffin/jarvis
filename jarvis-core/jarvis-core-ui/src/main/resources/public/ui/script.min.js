angular.module("JarvisApp","ngMaterial ngMdIcons ngSanitize restangular angular-websocket ui.router ui.router.router pascalprecht.translate ngCookies hljs ui.tree JarvisApp.websocket JarvisApp.config JarvisApp.theme JarvisApp.routes JarvisApp.services JarvisApp.services.filter JarvisApp.services.generic JarvisApp.services.scope JarvisApp.services.plugin JarvisApp.services.scenario JarvisApp.services.snapshot JarvisApp.services.cron JarvisApp.services.view JarvisApp.services.property JarvisApp.services.device JarvisApp.services.event JarvisApp.services.block JarvisApp.directives.files JarvisApp.directives.widgets jarvis.directives.navigator jarvis.directives.command jarvis.directives.trigger jarvis.directives.configuration jarvis.directives.notification jarvis.directives.connector JarvisApp.ctrl.plugins JarvisApp.ctrl.devices JarvisApp.ctrl.events JarvisApp.ctrl.snapshots JarvisApp.ctrl.crons JarvisApp.ctrl.views JarvisApp.ctrl.properties JarvisApp.ctrl.scenarios JarvisApp.ctrl.blocks JarvisApp.ctrl.home".split(" "));
...
function(a){l(a)}):b.all("/api/"+f[0]).one(f[1],g).all(c[0]).all(c[1]).getList({href:e}).then(q,function(a){l(a)})},post:function(c,e,g,n,k,l,q,p){var r=function(b){b=d.plain(b);a.debug("[POST/L]",c,b);void 0!=q&&q(b)};void 0!=l&&(l.href=g);1==e.length?1==c.length?b.one("/api/"+c[0],n).one(e[0],k).customPOST(l).then(r,function(a){p(a)}):b.all("/api/"+c[0]).one(c[1],n).one(e[0],k).customPOST(l).then(r,function(a){p(a)}):1==c.length?b.one("/api/"+c[0],n).all(e[0]).one(e[1],k).customPOST(l).then(r,function(a){p(a)}):
b.all("/api/"+c[0]).one(c[1],n).all(e[0]).one(e[1],k).customPOST(l).then(r,function(a){p(a)})},put:function(c,e,g,n,k,l,q,p,r){var t=function(b){b=d.plain(b);a.debug("[PUT/L]",c,b);void 0!=p&&p(b)},s={},s=void 0===q?{}:q;s.href=g;1==e.length?1==c.length?b.one("/api/"+c[0],n).one(e[0],k).one(l).customPUT(s).then(t,function(a){r(a)}):b.all("/api/"+c[0]).one(c[1],n).one(e[0],k).one(l).customPUT(s).then(t,function(a){r(a)}):1==c.length?b.one("/api/"+c[0],n).all(e[0]).one(e[1],k).one(l).customPUT(s).then(t,
function(a){r(a)}):b.all("/api/"+c[0]).one(c[1],n).all(e[0]).one(e[1],k).one(l).customPUT(s).then(t,function(a){r(a)})},delete:function(c,e,g,n,k,l,q,p){var r=function(b){b=d.plain(b);a.debug("[DELETE/L]",c,b);void 0!=q&&q(b)};1==e.length?1==c.length?b.one("/api/"+c[0],n).one(e[0],k).remove({instance:l,href:g}).then(r,function(a){p(a)}):b.all("/api/"+c[0]).one(c[1],n).one(e[0],k).remove({instance:l,href:g}).then(r,function(a){p(a)}):1==c.length?b.one("/api/"+c[0],n).all(e[0]).one(e[1],k).remove({instance:l,
href:g}).then(r,function(a){p(a)}):b.all("/api/"+c[0]).one(c[1],n).all(e[0]).one(e[1],k).remove({instance:l,href:g}).then(r,function(a){p(a)})}};return{crud:function(a){return{findAll:function(b,c){return e.findAll(a,b,c)},get:function(b,c,d){return e.get(a,b,c,d)},delete:function(b,c,d){return e.delete(a,b,c,d)},put:function(b,c,d){return e.put(a,b,c,d)},post:function(b,c,d){return e.post(a,b,c,d)},task:function(b,c,d,g,l){return e.task(a,b,c,d,g,l)}}},links:function(a,b,c){void 0===c&&(c="HREF");
...
angular.module("JarvisApp.services.plugin",[]).factory("pluginResourceService",["$log","Restangular","filterService","jarvisWidgetCommandService","genericResourceService",function(a,b,d,c,e){return{plugins:{findAll:function(a,c){var e=[],m=["scripts"],n=_.after(m.length,function(b){a(b)});_.forEach(m,function(a){b.all("plugins").all(a).getList().then(function(a){_.forEach(a,function(a){e.push(d.plain(a))});n(e)},function(a){c(a)})})}},scripts:e.crud(["plugins","scripts"]),commands:e.links(["plugins",
...
angular.module("JarvisApp.services.snapshot",[]).factory("snapshotResourceService",["genericResourceService",function(a){return{snapshot:a.crud(["snapshots"])}}]);"use strict";angular.module("JarvisApp.services.cron",[]).factory("cronResourceService",["genericResourceService",function(a){return{cron:a.crud(["crons"])}}]);"use strict";
angular.module("JarvisApp.services.device",[]).factory("deviceResourceService",["genericResourceService",function(a){return{device:a.crud(["devices"]),plugins:a.links(["devices"],["plugins","scripts"]),devices:a.links(["devices"],["devices"]),triggers:a.links(["devices"],["triggers"])}}]);"use strict";angular.module("JarvisApp.services.event",[]).factory("eventResourceService",["genericResourceService",function(a){return{event:a.crud(["events"])}}]);"use strict";
...
{url:"/snapshots/:id",controller:"snapshotCtrl",templateUrl:"/ui/js/partials/snapshots/snapshot/page.html"}).state("crons",{url:"/crons",controller:"cronsCtrl",templateUrl:"/ui/js/partials/crons/page.html"}).state("crons-by-id",{url:"/crons/:id",controller:"cronCtrl",templateUrl:"/ui/js/partials/crons/cron/page.html"}).state("scenarios",{url:"/scenarios",controller:"scenariosCtrl",templateUrl:"/ui/js/partials/scenarios/page.html"}).state("scenarios-by-id",{url:"/scenarios/:id?tab",controller:"scenarioCtrl",
templateUrl:"/ui/js/partials/scenarios/scenario/page.html"}).state("blocks",{url:"/blocks",controller:"blocksCtrl",templateUrl:"/ui/js/partials/blocks/page.html"}).state("blocks-by-id",{url:"/blocks/:id?tab",controller:"blockCtrl",templateUrl:"/ui/js/partials/blocks/block/page.html"})}]);"use strict";
...
"jarvisWidgetCommandService","pluginResourceService","deviceResourceService",function(a,b,d,c,e,g,f,h,m,n,k,l,q,p){return{devices:function(){var c=a.defer();p.device.findAll(function(a){_.each(a,function(a){p.plugins.findAll(a.id,function(b){a.desc="";_.each(b,function(b){a.desc+=b.name})});a.selectable=!0;a.callback=function(a){b.path("/devices/"+a.id)};a.ext=angular.toJson(angular.fromJson(a.parameters),!0)});c.resolve({elements:a,title:"Devices",route:"/devices"})});return c.promise},plugins:function(){var c=
...
function(a){a.selectable=!0;a.callback=function(a){b.path("/scenarios/"+a.id)}});c.resolve({elements:a,title:"Scenarios",route:"/scenarios"})});return c.promise},crons:function(){var c=a.defer();n.cron.findAll(function(a){_.each(a,function(a){a.selectable=!0;a.callback=function(a){b.path("/crons/"+a.id)}});c.resolve({elements:a,title:"Crontab",route:"/crons"})});return c.promise},connectors:function(){var c=a.defer();k.connector.findAll(function(a){_.each(a,function(a){a.selectable=!0;a.callback=
...
a.stop=function(){_.each(a.crons,function(a){c.cron.task(a.id,"toggle",{target:!1},function(a){e.info("crontab "+crontab.name+"#"+crontab.id+" toggled to "+crontab.status)},e.failure)})}}]).controller("cronCtrl",["$scope","$log","$stateParams","$filter","$http","genericResourceService","genericScopeService","cronResourceService","deviceResourceService","toastService",function(a,b,d,c,e,g,f,h,m,n){f.scope.resource(a,"cron","crons",h.cron);a.toggle=function(a){b.info(a);h.cron.task(a.id,"toggle",{},
function(a){n.info("crontab "+crontab.name+"#"+crontab.id+" toggled to "+crontab.status)},n.failure)};a.test=function(a){b.info(a);h.cron.task(a.id,"test",{},function(a){n.info("crontab "+crontab.name+"#"+crontab.id+" toggled to "+crontab.status)},n.failure)};a.load=function(){a.crons=[];g.scope.entity.get(d.id,function(b){a.cron=b},h.cron);b.info("cron-ctrl",a.crons)}}]);"use strict";
...
"jarvisWidgetCommandService","toastService",function(a,b,d,c,e,g,f,h,m,n){e.scope.resource(a,"script","plugins",f.scripts);a.links={commands:{}};e.scope.resourceLink(function(){return a.commands},a.links.commands,"script","plugins",f.scripts,f.commands,{order:"1",name:"noname",nature:"json",type:"data"},d.id);a.execute=function(c){void 0!=c&&void 0!=c.id&&""!=c.id&&f.scripts.task(c.id,"execute",a.rawTestData,function(d){b.debug("pluginScriptCtrl::execute",c,d);a.rawoutput=d;a.output=angular.toJson(d,
!0)},n.failure)};a.render=function(c){void 0!=c&&void 0!=c.id&&""!=c.id&&f.scripts.task(c.id,"render",a.rawTestData,function(d){b.debug("pluginScriptCtrl::render",c,d);a.rawoutput=d;a.output=angular.toJson(d,!0)},n.failure)};a.submit=function(){a.rawTestData=angular.fromJson(a.editTestData);a.jsonTestData=angular.toJson(a.rawTestData,!0)};a.clear=function(){a.rawoutput={};a.output=angular.toJson({},!0);a.rawTestData={};a.editTestData=angular.toJson(a.rawTestData,!0);a.jsonTestData=angular.toJson(a.rawTestData,
...
"toastService",function(a,b,d,c,e,g,f,h,m,n){f.scope.resource(a,"snapshot","snapshots",h.snapshot);a.restore=function(a){void 0!=a&&void 0!=a.id&&""!=a.id&&h.snapshot.task(a.id,"restore",{},function(c){b.debug("snapshotCtrl::restore",a,c)},n.failure)};a.download=function(a){void 0!=a&&void 0!=a.id&&""!=a.id&&h.snapshot.task(a.id,"download",{},function(d){var e="export-"+c("date")(new Date,"yyyyMMdd-HHmmss")+".json",f=document.createElement("a");document.body.appendChild(f);f.style="display: none";
b.debug("snapshot",a,d);d=new Blob([angular.toJson(d,!0)],{type:"application/text"});d=window.URL.createObjectURL(d);f.href=d;f.download=e;f.click()},n.failure)};a.upload=function(a){$("#"+a).trigger("click")};a.loaded=function(a,c){a.json=c.data;b.debug("loaded",a,c)};a.load=function(){a.snapshots=[];g.scope.entity.get(d.id,function(b){a.snapshot=b},h.snapshot);b.info("snapshot-ctrl",a.snapshots)}}]);"use strict";
...
"pluginResourceService","toastService",function(a,b,d,c,e,g,f,h,m,n,k){e.scope.resource(a,"scenario","scenarios",h.scenario);a.links={blocks:{},triggers:{}};e.scope.resourceLink(function(){return a.blocks},a.links.blocks,"scenario","scenarios",h.scenario,h.blocks,{order:"1"},d.id);e.scope.resourceLink(function(){return a.triggers},a.links.triggers,"scenario","scenarios",h.scenario,h.triggers,{order:"1"},d.id);a.build=function(a){var c=[];_.each(a,function(a){b.debug("graph",a.stage);var d="";_.each(a.nodes,
...
[];m.blocks.then.findAll(a.id,function(b){a.blocks.then=b});m.blocks.else.findAll(a.id,function(b){a.blocks.else=b});b.debug("blocks.plugins",a.plugins);b.debug("blocks.blocks",a.blocks)})});a.triggers=[];g.scope.collections.findAll("triggers",d.id,a.triggers,h.triggers);b.debug("scenario-ctrl",a.scenario)}}]);"use strict";
