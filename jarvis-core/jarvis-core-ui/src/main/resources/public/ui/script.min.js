...
b.show(b.simple().position(d.getToastPosition()).textContent(c).hideDelay(3E3).capsule(!0))}}}]);myAppServices.factory("clientResourceService",["$q","$window","$rootScope","Restangular",function(a,b,d,c){angular.injector(["ng"]).get("$log").info("clientResourceService",a);return{findAll:function(a,b){c.all("clients").getList().then(function(b){a(b)},function(a){b(a)})}}}]);
myAppServices.factory("crontabResourceService",["$q","$window","$rootScope","Restangular",function(a,b,d,c){angular.injector(["ng"]).get("$log").info("crontabResourceService",a);return{findAll:function(a,b){c.all("crontabs").getList().then(function(b){a(b)},function(a){b(a)})}}}]);
myAppServices.factory("oauth2ResourceService",["$rootScope","$log","$window","$mdDialog","$location","Restangular",function(a,b,d,c,e,g){b=angular.injector(["ng"]).get("$log");return{me:function(a,d,c){b.info("Fix JarvisAuthToken to",a);g.setDefaultHeaders({JarvisAuthToken:a});g.one("/api/profile/me").get().then(function(a){d(a)},function(a){c(a)})},config:function(a,b,d){g.one("oauth2").get(a).then(function(a){b(a)},function(a){d(a)})},connect:function(d,h){b.info("connect with",h);var p=this;p.me(h,
function(h){b.info("profile",h);c.hide();a.profile=h;d.boot();b.info("Switch to home");e.path("/home")},function(a){b.warn("no profile");p.login()})},login:function(){c.show({controller:"oauth2DialogCtrl",templateUrl:"/ui/js/partials/dialog/oauth2Dialog.tmpl.html",parent:angular.element(document.body),clickOutsideToClose:!1,fullscreen:!1})}}}]);"use strict";myAppServices=angular.module("JarvisApp.services.filter",[]);
...
f[0],c.id).customPUT(c).then(k,function(a){g(a)}):b.all("/api/"+f[0]).one(f[1],c.id).customPUT(c).then(k,function(a){g(a)})},post:function(f,c,e,g){var k=function(b){b=d.plain(b);a.debug("[POST]",f,b);void 0!=e&&e(b)};1==f.length?b.all("/api/"+f[0]).post(c).then(k,function(a){g(a)}):b.all("/api/"+f[0]).all(f[1]).post(c).then(k,function(a){g(a)})},task:function(f,c,e,g,k,l){var m=function(b){var c=d.plain(b);a.debug("[TASK]",b,f,c);void 0!=k&&k(c)};1==f.length?b.all("/api/"+f[0]).one(c).customPOST(g,
"",{task:e}).then(m,function(a){l(a)}):b.all("/api/"+f[0]).all(f[1]).one(c).customPOST(g,"",{task:e}).then(m,function(a){l(a)})}},g={findAll:function(f,c,e,g,k,l){var m=function(b){var l=[];_.forEach(b,function(a){l.push(d.plain(a))});a.debug("[FIND/L/"+e+"]",f,c,g,l);k(l)};1==c.length?1==f.length?b.one("/api/"+f[0],g).all(c[0]).getList({href:e}).then(m,function(a){l(a)}):b.all("/api/"+f[0]).one(f[1],g).all(c[0]).getList({href:e}).then(m,function(a){l(a)}):1==f.length?b.one("/api/"+f[0],g).all(c[0]).all(c[1]).getList({href:e}).then(m,
function(a){l(a)}):b.all("/api/"+f[0]).one(f[1],g).all(c[0]).all(c[1]).getList({href:e}).then(m,function(a){l(a)})},post:function(f,c,e,g,k,l,m,q){var r=function(b){b=d.plain(b);a.debug("[POST/L]",f,b);void 0!=m&&m(b)};void 0!=l&&(l.href=e);1==c.length?1==f.length?b.one("/api/"+f[0],g).one(c[0],k).customPOST(l).then(r,function(a){q(a)}):b.all("/api/"+f[0]).one(f[1],g).one(c[0],k).customPOST(l).then(r,function(a){q(a)}):1==f.length?b.one("/api/"+f[0],g).all(c[0]).one(c[1],k).customPOST(l).then(r,function(a){q(a)}):
b.all("/api/"+f[0]).one(f[1],g).all(c[0]).one(c[1],k).customPOST(l).then(r,function(a){q(a)})},put:function(c,e,g,n,k,l,m,q,r){var t=function(b){b=d.plain(b);a.debug("[PUT/L]",c,b);void 0!=q&&q(b)},s={},s=void 0===m?{}:m;s.href=g;1==e.length?1==c.length?b.one("/api/"+c[0],n).one(e[0],k).one(l).customPUT(s).then(t,function(a){r(a)}):b.all("/api/"+c[0]).one(c[1],n).one(e[0],k).one(l).customPUT(s).then(t,function(a){r(a)}):1==c.length?b.one("/api/"+c[0],n).all(e[0]).one(e[1],k).one(l).customPUT(s).then(t,
function(a){r(a)}):b.all("/api/"+c[0]).one(c[1],n).all(e[0]).one(e[1],k).one(l).customPUT(s).then(t,function(a){r(a)})},delete:function(c,e,g,n,k,l,m,q){var r=function(b){b=d.plain(b);a.debug("[DELETE/L]",c,b);void 0!=m&&m(b)};1==e.length?1==c.length?b.one("/api/"+c[0],n).one(e[0],k).remove({instance:l,href:g}).then(r,function(a){q(a)}):b.all("/api/"+c[0]).one(c[1],n).one(e[0],k).remove({instance:l,href:g}).then(r,function(a){q(a)}):1==c.length?b.one("/api/"+c[0],n).all(e[0]).one(e[1],k).remove({instance:l,
href:g}).then(r,function(a){q(a)}):b.all("/api/"+c[0]).one(c[1],n).all(e[0]).one(e[1],k).remove({instance:l,href:g}).then(r,function(a){q(a)})}};return{crud:function(a){return{findAll:function(b,c){return e.findAll(a,b,c)},get:function(b,c,d){return e.get(a,b,c,d)},delete:function(b,c,d){return e.delete(a,b,c,d)},put:function(b,c,d){return e.put(a,b,c,d)},post:function(b,c,d){return e.post(a,b,c,d)},task:function(b,c,d,g,l){return e.task(a,b,c,d,g,l)}}},links:function(a,b,c){void 0===c&&(c="HREF");
return{findAll:function(d,e,l){return g.findAll(a,b,c,d,e,l)},delete:function(d,e,l,m,q){return g.delete(a,b,c,d,e,l,m,q)},put:function(d,e,l,m,q,r){return g.put(a,b,c,d,e,l,m,q,r)},post:function(d,e,l,m,q){return g.post(a,b,c,d,e,l,m,q)}}},scope:{collections:{new:function(b,d,e,g,k){g.post(e,function(c){a.debug(b+" "+c.name+"#"+c.id+" created");d.push(c);k&&k(c)},c.failure)},findAll:function(b,d,e,g,k){a.debug("loading ",b,g);e.splice(0,e.length);g.findAll(d,function(c){_.forEach(c,function(a){e.push(a)});
...
angular.module("JarvisApp.services.scope",["JarvisApp.services.generic"]).factory("genericScopeService",["$log","$filter","genericResourceService","genericPickerService","toastService",function(a,b,d,c,e){return{scope:{resources:function(g,f,h,p,n,k){a.debug("inject default resources scope",p);h.create=function(a,b){d.scope.collections.new(p,f(),a,n,b)};h.new=function(a){d.scope.collections.new(p,f(),k,n,a)};h.crud=d.crud(p.split("/"));h.remove=function(d,e){c.pickers.confirm(b("translate")("action.drop",
{name:d.name,id:d.id}),"",b("translate")("yes"),b("translate")("no")).then(function(){a.debug("remove",d);var b=d.id;_.remove(f(),function(a){return a.id==b});h.crud.delete(d.id);e&&e(d)},function(){a.debug("abort")})};h.load=function(b){g([]);n.findAll(function(d){g(d);a.debug(p+"-ctrl",d);b&&b()},e.failure)}},resource:function(e,f,h,p,n,k,l){a.debug("inject default resource scope",f);e.remove=function(k){c.pickers.confirm(b("translate")("action.drop",{name:k.name,id:k.id}),"",b("translate")("yes"),
b("translate")("no")).then(function(){d.scope.entity.remove(function(){e.go(h)},f,k,p)},function(){a.debug("abort")})};e.save=function(a){d.scope.entity.save(f,a,p)};e.duplicate=function(a){d.scope.entity.duplicate(function(){e.go(h)},f,a,p)}},resourceLink:function(e,f,h,p,n,k,l,m,q){a.debug("inject link",k);f.add=function(b){a.debug("link::add",b);d.scope.link.add(m,b,l,k,e())};f.update=function(b){a.debug("link::update",b);d.scope.link.save(m,b,k)};f.drop=function(f){c.pickers.confirm(b("translate")("action.remove.link",
{name:f.name,id:f.id}),"",b("translate")("yes"),b("translate")("no")).then(function(){a.debug("link::drop",f);q&&q.remove?q.remove(f):d.scope.link.remove(m,f,k,e());q&&q.afterRemove&&q.afterRemove(f)},function(){a.debug("abort")})}}}}}]);"use strict";
angular.module("JarvisApp.services.plugin",[]).factory("pluginResourceService",["$log","Restangular","filterService","commandResourceService","genericResourceService",function(a,b,d,c,e){return{plugins:{findAll:function(a,c){var e=[],p=["scripts"],n=_.after(p.length,function(b){a(b)});_.forEach(p,function(a){b.all("plugins").all(a).getList().then(function(a){_.forEach(a,function(a){e.push(d.plain(a))});n(e)},function(a){c(a)})})}},scripts:e.crud(["plugins","scripts"]),commands:e.links(["plugins",
...
angular.module("JarvisApp.services.block",[]).factory("blockResourceService",["$log","Restangular","genericResourceService","iotResourceService","filterService",function(a,b,d,c,e){return{block:d.crud(["blocks"]),plugins:{"if":d.links(["blocks"],["plugins","scripts"],"HREF_IF"),then:d.links(["blocks"],["plugins","scripts"],"HREF_THEN"),"else":d.links(["blocks"],["plugins","scripts"],"HREF_ELSE")},blocks:{then:d.links(["blocks"],["blocks"],"HREF_THEN"),"else":d.links(["blocks"],["blocks"],"HREF_ELSE")}}}]);
"use strict";
angular.module("JarvisApp.config",[]).config(["RestangularProvider",function(a){a.setDefaultHeaders({"content-type":"application/json"});a.setFullRequestInterceptor(function(a,d,c,e,g,f,h){return{element:a,params:f,headers:g,httpConfig:h}});a.setResponseExtractor(function(a){angular.isObject(a)&&(a.originalElement=a);return a})}]).config(["$mdIconProvider",function(a){}]).config(["$translateProvider",function(a){a.useStaticFilesLoader({prefix:"js/l10n/",suffix:".json"});a.preferredLanguage("fr_FR");a.useSanitizeValueStrategy(null)}]).controller("JarvisAppCtrl",
["$rootScope","$scope","$log","$store","$http","$mdDialog","$mdSidenav","$mdMedia","$location","$window","$state","genericPickerService","toastService","iotResourceService","eventResourceService","configurationResourceService","oauth2ResourceService",function(a,b,d,c,e,g,f,h,p,n,k,l,m,q,r,t,s){b.defaultValue=function(a,b){return void 0===a?b:a};b.loadSettings=function(){t.configuration.findAll(function(a){if(a=_.find(a,"active"))b.config=a},m.failure)};b.saveSettings=function(){t.configuration.put(b.config,
function(a){d.info("Updated",a)},m.failure)};b.closeSettings=function(){g.hide()};b.settings=function(a){var c=h("sm")||h("xs");g.show({scope:b,preserveScope:!0,templateUrl:"js/partials/dialog/settingsDialog.tmpl.html",parent:angular.element(document.body),targetEvent:a,clickOutsideToClose:!0,fullscreen:c}).then(function(){d.warn("Saving settings")},function(){d.warn("Cancel settings save")})};b.markdown=new showdown.Converter({tables:"true",ghCodeBlocks:"true"});b.helper=function(a){d.debug("State",
k.current.name);e.get("js/helps/fr/"+k.current.name+".markdown").then(function(a){a=b.markdown.makeHtml(a.data);b.help={content:a.replace(/<table>/g,'\x3ctable class\x3d"table table-striped table-bordered table-hover"\x3e')};d.debug("Help",b.help);f("right").toggle()})};b.toggleSidenav=function(a){f(a).toggle()};b.location=function(a,b){f(a).toggle();p.path(b)};b.go=function(a,b){k.go(a,b)};b.emit=function(a,b){d.debug("JarvisAppCtrl::emit",a,b,a.trigger);r.event.post({trigger:a.trigger,timestamp:(new Date).toISOString(),
fired:!0,number:b},function(a){d.debug("JarvisAppCtrl::emit",a);m.info("event "+a.trigger+"#"+a.id+" emitted")},m.failure)};b.execute=function(a){q.iot.task(a.id,"execute",{},function(a){d.debug("JarvisAppCtrl::execute",a);b.renderdata=a},m.failure)};b.graph=function(a,b){return l.pickers.graph(a,b,function(a){d.debug("Go")},function(){d.debug("Abort")},"graphDialogCtrl")};b.boot=function(){d.info("JarvisAppCtrl");b.config={};b.media=h("xs");b.$watch(function(){return h("xs")},function(a){a&&(b.media=
"xs")});b.$watch(function(){return h("gt-xs")},function(a){a&&(b.media="gt-xs")});b.$watch(function(){return h("sm")},function(a){a&&(b.media="sm")});b.$watch(function(){return h("gt-sm")},function(a){a&&(b.media="gt-sm")});b.$watch(function(){return h("md")},function(a){a&&(b.media="md")});b.$watch(function(){return h("gt-md")},function(a){a&&(b.media="gt-md")});b.$watch(function(){return h("lg")},function(a){a&&(b.media="lg")});b.$watch(function(){return h("gt-lg")},function(a){a&&(b.media="gt-lg")});
b.$watch(function(){return h("xl")},function(a){a&&(b.media="xl")});b.$watch(function(){return h("print")},function(a){a&&(b.media="print")});hljs.initHighlightingOnLoad();b.loadSettings();d.info("JarvisAppCtrl configured")};s.connect(b)}]).controller("extractTokenCtrl",["$scope","$log","$location","$rootScope","$state","oauth2ResourceService",function(a,b,d,c,e,g){b.warn("extractTokenCtrl",e);b=d.path().substr(1).split("\x26");d={};for(c=0;c<b.length;c++){e=b[c].split("\x3d");var f=e[0];d[f]=e[1];
"access_token"===f&&g.connect(a,d.access_token)}}]).controller("oauth2DialogCtrl",["$scope","$window","$log","$mdDialog","oauth2ResourceService",function(a,b,d,c,e){a.hide=function(){c.hide()};a.cancel=function(){c.cancel()};a.answer=function(a){e.config({client:a.client,oauth2_redirect_uri:b.location.href.split("#")[0]},function(a){d.info("JarvisAppCtrl configured with",a);b.location.href=a.url},function(a){d.warn("no oauth2 configuration",a)});c.hide()}}]).controller("graphDialogCtrl",["$scope",
"$log","$mdDialog",function(a,b,d){b.info("graphDialogCtrl");a.hide=function(){d.hide()};a.cancel=function(){d.cancel()};a.answer=function(a){d.hide(a)}}]).controller("pickPluginDialogCtrl",["$scope","$log","$mdDialog","genericResourceService","toastService",function(a,b,d,c,e){b.info("pickPluginDialogCtrl");a.hide=function(){d.hide()};a.cancel=function(){d.cancel()};a.answer=function(a){d.hide(a)};a.elementsPicker=[{name:"Connected Objects",selectable:!1,nodes:[]},{name:"Plugin Scripts",selectable:!1,
nodes:[]}];a.crudIot=c.crud(["iots"]);a.crudIot.findAll(function(b){_.each(b,function(b){b.selectable=!1;a.elementsPicker[0].nodes.push(b)})},e.failure);a.crudScript=c.crud(["plugins","scripts"]);a.crudScript.findAll(function(b){_.each(b,function(b){b.selectable=!0;a.elementsPicker[1].nodes.push(b)})},e.failure)}]).controller("pickIotDialogCtrl",["$scope","$log","$mdDialog","genericResourceService","toastService",function(a,b,d,c,e){b.info("pickIotDialogCtrl");a.hide=function(){d.hide()};a.cancel=
function(){d.cancel()};a.answer=function(a){d.hide(a)};a.elementsPicker=[{name:"Connected Objects",selectable:!1,nodes:[]},{name:"Plugin Scripts",selectable:!1,nodes:[]}];a.crudIot=c.crud(["iots"]);a.crudIot.findAll(function(b){_.each(b,function(b){b.selectable=!0;a.elementsPicker[0].nodes.push(b)})},e.failure);a.crudScript=c.crud(["plugins","scripts"]);a.crudScript.findAll(function(b){_.each(b,function(b){b.selectable=!1;a.elementsPicker[1].nodes.push(b)})},e.failure)}]).controller("pickCommandDialogCtrl",
["$scope","$log","$mdDialog","genericResourceService","toastService",function(a,b,d,c,e){b.info("pickCommandDialogCtrl");a.hide=function(){d.hide()};a.cancel=function(){d.cancel()};a.answer=function(a){d.hide(a)};a.elementsPicker=[{name:"Commands",selectable:!1,nodes:[]}];a.crud=c.crud(["commands"]);a.crud.findAll(function(b){_.each(b,function(b){b.selectable=!0;a.elementsPicker[0].nodes.push(b)})},e.failure)}]).controller("pickTriggerDialogCtrl",["$scope","$log","$mdDialog","genericResourceService",
"toastService",function(a,b,d,c,e){b.info("pickTriggerDialogCtrl");a.hide=function(){d.hide()};a.cancel=function(){d.cancel()};a.answer=function(a){d.hide(a)};a.elementsPicker=[{name:"Triggers",selectable:!1,nodes:[]}];a.crud=c.crud(["triggers"]);a.crud.findAll(function(b){_.each(b,function(b){b.selectable=!0;a.elementsPicker[0].nodes.push(b)})},e.failure)}]).controller("pickCronDialogCtrl",["$scope","$log","$mdDialog","genericResourceService","toastService",function(a,b,d,c,e){b.info("pickCronDialogCtrl");
a.hide=function(){d.hide()};a.cancel=function(){d.cancel()};a.answer=function(a){d.hide(a)};a.elementsPicker=[{name:"Crontab objects",selectable:!1,nodes:[]}];a.crud=c.crud(["crons"]);a.crud.findAll(function(b){_.each(b,function(b){b.selectable=!0;a.elementsPicker[0].nodes.push(b)})},e.failure)}]).controller("pickBlockDialogCtrl",["$scope","$log","$mdDialog","genericResourceService","toastService",function(a,b,d,c,e){b.info("pickBlockDialogCtrl");a.hide=function(){d.hide()};a.cancel=function(){d.cancel()};
a.answer=function(a){d.hide(a)};a.elementsPicker=[{name:"Blocks",selectable:!1,nodes:[]}];a.crudBlock=c.crud(["blocks"]);a.crudBlock.findAll(function(b){_.each(b,function(b){b.selectable=!0;a.elementsPicker[0].nodes.push(b)})},e.failure)}]);"use strict";
...
angular.module("JarvisApp.websocket",["angular-websocket"]).factory("$store",["$log","$websocket","$window",function(a,b,d){a.info("Websocket: ",d.location);var c=b("ws://"+d.location.hostname+":"+d.location.port+"/stream/"),e={};c.onMessage(function(a){a=JSON.parse(a.data);void 0===e[a.classname]&&(e[a.classname]={});e[a.classname][a.instance]=a.data});return{collection:e,get:function(){c.send(JSON.stringify({action:"get"}))}}}]);"use strict";
...
"use strict";
angular.module("JarvisApp.directives",[]).directive("appVersion",function(a){return function(b,d,c){d.text(a)}}).directive("fileModel",function(a,b){return{restrict:"A",link:function(b,c,e){var g=a(e.fileModel).assign;c.bind("change",function(){b.$apply(function(){g(b,c[0].files[0])})})}}}).directive("fileSelect",function(a,b){return{restrict:"A",require:"ngModel",link:function(d,c,e,g){var f=new a.FileReader;f.onload=function(){var a=f.result.substr(f.result.lastIndexOf(",")+1);b.debug("result:",f.result);
b.debug("result base64 value:",a);g.$setViewValue(atob(a));"fileLoaded"in e&&d.$eval(e.fileLoaded)};f.onprogress=function(a){"fileProgress"in e&&d.$eval(e.fileProgress,{$total:a.total,$loaded:a.loaded})};f.onerror=function(){"fileError"in e&&d.$eval(e.fileError,{$error:f.error})};var h=e.fileSelect;c.bind("change",function(a){a=a.target.files[0];""===h||"text"===h?f.readAsText(a,"UTF-8"):"data"===h&&f.readAsDataURL(a)})}}});"use strict";
...
function(a,b){return{restrict:"E",templateUrl:"/ui/js/partials/plugins/script/jarvis-plugin-result.html",link:function(b,c,e){a.debug("jarvis-plugin-output")}}}]).directive("jarvisIots",["$log","$stateParams",function(a,b){return{restrict:"E",templateUrl:"/ui/js/partials/iots/jarvis-iots.html",link:function(b,c,e){a.debug("jarvis-iots")}}}]).directive("jarvisIot",["$log","$stateParams",function(a,b){return{restrict:"E",templateUrl:"/ui/js/partials/iots/iot/jarvis-iot-general.html",link:function(b,
c,e){a.debug("jarvis-iot-general")}}}]).directive("jarvisIotPlugin",["$log","$stateParams",function(a,b){return{restrict:"E",templateUrl:"/ui/js/partials/iots/iot/jarvis-iot-plugin.html",link:function(b,c,e){a.debug("jarvis-iot-plugin")}}}]).directive("jarvisIotRender",["$log","$stateParams",function(a,b){return{restrict:"E",templateUrl:"/ui/js/partials/iots/iot/jarvis-iot-render.html",link:function(b,c,e){a.debug("jarvis-iot-render")}}}]).directive("jarvisEvents",["$log","$stateParams",function(a,
b){return{restrict:"E",templateUrl:"/ui/js/partials/events/jarvis-events.html",link:function(b,c,e){a.debug("jarvis-iots")}}}]).directive("jarvisTriggers",["$log","$stateParams",function(a,b){return{restrict:"E",templateUrl:"/ui/js/partials/triggers/jarvis-triggers.html",link:function(b,c,e){a.debug("jarvis-triggers")}}}]).directive("jarvisTrigger",["$log","$stateParams",function(a,b){return{restrict:"E",templateUrl:"/ui/js/partials/triggers/trigger/jarvis-trigger-general.html",link:function(b,c,
e){a.debug("jarvis-trigger")}}}]).directive("jarvisTriggerCron",["$log","$stateParams",function(a,b){return{restrict:"E",templateUrl:"/ui/js/partials/triggers/trigger/jarvis-trigger-cron.html",link:function(b,c,e){a.debug("jarvis-trigger-cron")}}}]).directive("jarvisCommands",["$log","$stateParams",function(a,b){return{restrict:"E",templateUrl:"/ui/js/partials/commands/jarvis-commands.html",link:function(b,c,e){a.debug("jarvis-commands")}}}]).directive("jarvisCommand",["$log","$stateParams",function(a,
b){return{restrict:"E",templateUrl:"/ui/js/partials/commands/command/jarvis-command-general.html",link:function(b,c,e){a.debug("jarvis-command")}}}]).directive("jarvisCommandInput",["$log","$stateParams",function(a,b){return{restrict:"E",templateUrl:"/ui/js/partials/commands/command/jarvis-command-input.html",link:function(b,c,e){a.debug("jarvis-command-input")}}}]).directive("jarvisCommandScript",["$log","$stateParams",function(a,b){return{restrict:"E",templateUrl:"/ui/js/partials/commands/command/jarvis-command-script.html",
link:function(b,c,e){a.debug("jarvis-command-script")}}}]).directive("jarvisCommandOutput",["$log","$stateParams",function(a,b){return{restrict:"E",templateUrl:"/ui/js/partials/commands/command/jarvis-command-output.html",link:function(b,c,e){a.debug("jarvis-command-output")}}}]).directive("jarvisViews",["$log","$stateParams",function(a,b){return{restrict:"E",templateUrl:"/ui/js/partials/views/jarvis-views.html",link:function(b,c,e){a.debug("jarvis-views")}}}]).directive("jarvisView",["$log","$stateParams",
function(a,b){return{restrict:"E",templateUrl:"/ui/js/partials/views/view/jarvis-view-general.html",link:function(b,c,e){a.debug("jarvis-view")}}}]).directive("jarvisConfigurations",["$log","$stateParams",function(a,b){return{restrict:"E",templateUrl:"/ui/js/partials/configurations/jarvis-configurations.html",link:function(b,c,e){a.debug("jarvis-configurations")}}}]).directive("jarvisConfiguration",["$log","$stateParams",function(a,b){return{restrict:"E",templateUrl:"/ui/js/partials/configurations/configuration/jarvis-configuration-general.html",
...
"$stateParams","$parse",function(a,b,d){return{restrict:"E",template:'\x3cdiv ng-include\x3d"getJarvisInlineTemplateUrl()"\x3e\x3c/div\x3e',link:function(b,e,g){a.debug("jarvis-inline-template id",d(g.id)(b));b.getJarvisInlineTemplateUrl=function(){b.data=d(g.data)(b);return"/api/directives/html/iots/"+d(g.id)(b)}}}}]).directive("jarvisGauge",["$log","$store",function(a,b){return{restrict:"AC",controller:["$scope","$element","$attrs",function(d,c,e){a.debug("jarvis-gauge",e.id);d.gauge=(new Gauge(document.getElementById(e.id))).setOptions({fontSize:10,
lines:10,angle:0,lineWidth:.32,pointer:{length:.9,strokeWidth:.035,color:"#000000"},limitMax:"false",colorStart:"#6FADCF",colorStop:"#8FC0DA",strokeColor:"#E0E0E0",generateGradient:!0});d.gauge.maxValue=100;d.gauge.animationSpeed=10;d.gauge.set(0);d.systemIndicator=function(){return void 0==b.collection||void 0==b.collection.SystemIndicator||void 0==b.collection.SystemIndicator["1"]?0:b.collection.SystemIndicator["1"]};d.$watch(d.systemIndicator,function(a,b,c){d.gauge.set(1E3*a.processCpuLoad)})}]}}]);
"use strict";
...
"execute",{},function(e){b.debug("[BLOCK/test]",c,e);a.testExpression=e},h.failure)};a.load=function(){a.activeTab=d.tab;e.scope.entity.get(d.id,function(b){a.block=b},f.block);a.plugins={if:{},then:{},else:{}};a.plugins.if=[];e.scope.collections.findAll("plugins.if",d.id,a.plugins.if,f.plugins.if);a.plugins.then=[];e.scope.collections.findAll("plugins.then",d.id,a.plugins.then,f.plugins.then);a.plugins.else=[];e.scope.collections.findAll("plugins.else",d.id,a.plugins.else,f.plugins.else);a.blocks=
...
angular.module("JarvisApp.ctrl.iots",["JarvisApp.services"]).controller("iotsCtrl",["$scope","$log","genericScopeService","iotResourceService",function(a,b,d,c){d.scope.resources(function(b){a.iots=b},function(){return a.iots},a,"iots",c.iot,{name:"object name",icon:"list"})}]).controller("iotCtrl",["$scope","$log","$state","$stateParams","iotResourceService","pluginResourceService","genericScopeService","genericResourceService","toastService",function(a,b,d,c,e,g,f,h,p){a.getLink=function(){return a.plugins};
f.scope.resource(a,"iot","iots",e.iot);a.links={plugins:{},triggers:{},iots:{}};f.scope.resourceLink(function(){return a.plugins},a.links.plugins,"iot","iots",e.iot,e.plugins,{order:"1"},c.id);f.scope.resourceLink(function(){return a.triggers},a.links.triggers,"iot","iots",e.iot,e.triggers,{order:"1"},c.id);f.scope.resourceLink(function(){return a.iots},a.links.iots,"iot","iots",e.iot,e.iots,{order:"1"},c.id);a.render=function(c){e.iot.task(c.id,"render",{},function(c){b.debug("iotCtrl::render",c);
a.renderdata=c;a.output=angular.toJson(c,!0)},p.failure)};a.load=function(){a.activeTab=c.tab;h.scope.entity.get(c.id,function(b){a.iot=b},e.iot);a.plugins=[];h.scope.collections.findAll("plugins",c.id,a.plugins,e.plugins);a.iots=[];h.scope.collections.findAll("iots",c.id,a.iots,e.iots);a.triggers=[];h.scope.collections.findAll("triggers",c.id,a.triggers,e.triggers);b.info("iot-ctrl")}}]);"use strict";
...
angular.module("JarvisApp.ctrl.plugins",["JarvisApp.services"]).controller("pluginsCtrl",["$scope","$log","genericScopeService","pluginResourceService",function(a,b,d,c){d.scope.resources(function(b){a.plugins=b},function(){return a.plugins},a,"plugins/scripts",c.scripts,{name:"script name",icon:"list",type:"script"})}]).controller("pluginScriptCtrl",["$scope","$log","$stateParams","genericResourceService","genericScopeService","genericPickerService","pluginResourceService","iotResourceService","commandResourceService",
"toastService",function(a,b,d,c,e,g,f,h,p,n){e.scope.resource(a,"script","plugins",f.scripts);a.links={commands:{}};e.scope.resourceLink(function(){return a.commands},a.links.commands,"script","plugins",f.scripts,f.commands,{order:"1",name:"noname",nature:"json",type:"data"},d.id);a.execute=function(c){void 0!=c&&void 0!=c.id&&""!=c.id&&f.scripts.task(c.id,"execute",a.rawTestData,function(d){b.debug("pluginScriptCtrl::execute",c,d);a.rawoutput=d;a.output=angular.toJson(d,!0)},n.failure)};a.render=
...
function(c){b.debug("setOwner",c);a.script.owner=c.id};a.load=function(){a.activeTab=d.tab;a.clear();a.combo={booleans:[{id:!0,value:"True"},{id:!1,value:"False"}]};c.scope.entity.get(d.id,function(b){a.script=b},f.scripts);a.commands=[];c.scope.collections.findAll("commands",d.id,a.commands,f.commands);a.combo.owners=[{id:void 0,name:"iot.empty"}];c.scope.combo.findAll("owner",a.combo.owners,h.iot);b.info("script-ctrl")}}]);"use strict";
angular.module("JarvisApp.ctrl.commands",["JarvisApp.services"]).controller("commandsCtrl",["$scope","$log","genericScopeService","commandResourceService",function(a,b,d,c){d.scope.resources(function(b){a.commands=b},function(){return a.commands},a,"commands",c.command,{name:"command name",icon:"list"})}]).controller("commandCtrl",["$scope","$log","$stateParams","genericResourceService","genericScopeService","commandResourceService","toastService",function(a,b,d,c,e,g,f){e.scope.resource(a,"command",
...
[];c.scope.entity.get(d.id,function(b){a.command=b},g.command);b.info("command-ctrl")}}]);"use strict";
angular.module("JarvisApp.ctrl.views",["JarvisApp.services"]).controller("viewsCtrl",["$scope","$log","genericScopeService","viewResourceService",function(a,b,d,c){b.info("viewsCtrl");d.scope.resources(function(b){a.views=b},function(){return a.views},a,"views",c.view,{name:"view name",icon:"list"})}]).controller("viewCtrl",["$scope","$log","$stateParams","genericResourceService","genericScopeService","viewResourceService","iotResourceService","toastService",function(a,b,d,c,e,g,f,h){e.scope.resource(a,
"view","views",g.view);a.links={iots:{}};e.scope.resourceLink(function(){return a.iots},a.links.iots,"view","views",g.view,g.iots,{order:"1"},d.id);a.load=function(){a.views=[];c.scope.entity.get(d.id,function(b){a.view=b},g.view);a.iots=[];c.scope.collections.findAll("iots",d.id,a.iots,g.iots);b.info("view-ctrl",a.views)}}]);"use strict";
...
"pluginResourceService","toastService",function(a,b,d,c,e,g,f,h,p,n,k){e.scope.resource(a,"scenario","scenarios",h.scenario);a.links={blocks:{},triggers:{}};e.scope.resourceLink(function(){return a.blocks},a.links.blocks,"scenario","scenarios",h.scenario,h.blocks,{order:"1"},d.id);e.scope.resourceLink(function(){return a.triggers},a.links.triggers,"scenario","scenarios",h.scenario,h.triggers,{order:"1"},d.id);a.build=function(a){var c=[];_.each(a,function(a){b.debug("graph",a.stage);var d="";_.each(a.nodes,
...
a.load=function(){a.activeTab=d.tab;a.plugin={};g.scope.entity.get(d.id,function(b){a.scenario=b;a.chart(b)},h.scenario);a.blocks=[];g.scope.collections.findAll("blocks",d.id,a.blocks,h.blocks,function(a){_.forEach(a,function(a){a.plugins={};a.plugins.if=[];a.plugins.then=[];a.plugins.else=[];p.plugins.if.findAll(a.id,function(b){a.plugins.if=b});p.plugins.then.findAll(a.id,function(b){a.plugins.then=b});p.plugins.else.findAll(a.id,function(b){a.plugins.else=b});a.blocks={};a.blocks.then=[];a.blocks.else=
[];p.blocks.then.findAll(a.id,function(b){a.blocks.then=b});p.blocks.else.findAll(a.id,function(b){a.blocks.else=b});b.debug("blocks.plugins",a.plugins);b.debug("blocks.blocks",a.blocks)})});a.triggers=[];g.scope.collections.findAll("triggers",d.id,a.triggers,h.triggers);b.debug("scenario-ctrl",a.scenario)}}]);