...
public class ApiDeviceResources extends
		ApiLinkedThirdResources<DeviceRest, DeviceBean, DeviceRest, DeviceBean, ScriptPluginRest, ScriptPluginBean, TriggerRest, TriggerBean> {
	...
	public GenericValue doRealTask(DeviceBean device, GenericMap args, String taskType) throws TechnicalException {
		...
		case "uml":
			try {
				return new GenericValue(ResultType.SVG, plantuml(device, args));
	 ...
	 */
	private String plantuml(DeviceBean device, GenericMap args) throws TechnicalNotFoundException {
		StringBuilder sb = new StringBuilder();
		sb.append("@startuml\n");
		sb.append("autonumber\n");
		for (GenericEntity entity : sort(apiHrefDeviceScriptPluginResources.findAll(device, HREF), "order")) {
			ScriptPluginBean plugin = apiScriptPluginResources.doGetByIdBean(entity.id);
			sb.append(MessageFormat.format("\"{0}\" -> \"{1}\" : appel\n", device.name, plugin.name, entity.get("type")));
			sb.append(MessageFormat.format("activate \"{0}\"\n", plugin.name));
			apiScriptPluginResources.loadFromChildren(sb, plugin);
	...
	private GenericMap renderOrExecute(DeviceBean device, GenericMap args, boolean render) throws Exception {
		...
		/**
		 * read parameters and fix a default value if parameters is null
		 */
		...
		for (Entry<String, Object> entry : parameters.entrySet()) {
			...
			for (GenericEntity entity : sort(apiHrefDeviceScriptPluginResources.findAll(device, HREF), "order")) {
				...
				if (render) {
					...
					result = apiScriptPluginResources.render(device,
							mapperFactory.getMapperFacade().map(script, ScriptPluginBean.class), params);
					logger.info("After render params = {}, context = {}", params, result);
				} else {
					logger.info("Before execute params = {}, context = {}", params, result);
					result = apiScriptPluginResources.execute(device,
							mapperFactory.getMapperFacade().map(script, ScriptPluginBean.class), params);
