...
angular.module('myImpressDocumentation.controllers', []).controller('ImpressCtrl', [ '$rootScope', '$window', function($scope, $window) {
	 ...
	 * </ul>
	 */
	$scope.editSlide = function(event, id) {
		var svg = $("#" + id).html();
		$window.open('svg-edit/svg-editor.html?source=' + encodeURIComponent('data:image/svg+xml;base64,' + $scope.tools.base64.encode(svg)));
		return false;
	}
	/**
	 * init steps
	...
	$scope.loadPresentation = function(rootId) {
		...
		$scope.arborInit({
			...
			gravity : true,
			dt : 0.015
		}, nodeModel, 'viewport', window.innerWidth, window.innerHeight)
	}
	...
	$scope.tools = {
		...
		base64 : {
			...
			encode : function(e) {
				...
				var n, r, i, s, o, u, a;
				var f = 0;
				e = $scope.tools.base64._utf8_encode(e);
				while (f < e.length) {
					n = e.charCodeAt(f++);
					r = e.charCodeAt(f++);
					i = e.charCodeAt(f++);
			...
			decode : function(e) {
				...
				t = $scope.tools.base64._utf8_decode(t);
				return t
			},
