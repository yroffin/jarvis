...
/**
 * An AudioFileDataSource generates a stream of audio data from a given audio
 * file. All required information concerning the audio format are read directly
 * from the file . One would need to call
 * {@link #setAudioFile(java.io.File,String)} to set the input file, and call
 * {@link #getData} to obtain the Data frames.
 * <p/>
 * Using JavaSound as backend this class is able to handle all sound files
 * supported by JavaSound. Beside the built-in support for .wav, .au and .aiff.
 * Using plugins (cf. http://www.jsresources.org/ ) it can be extended to
 * support .ogg, .mp3, .speex and others.
 * 
...
public class AudioFileDataSource extends BaseDataProcessor {
	...
	public void newProperties(PropertySheet ps) throws PropertyException {
		...
		create(ps.getInt(PROP_BYTES_PER_READ), ps.getComponentList(
				AUDIO_FILE_LISTENERS, AudioFileProcessListener.class));
	}
	 ...
	 * 
	 * @param audioFile
	 *            The location of the audio file to use
	 * @param streamName
	 *            The name of the InputStream. if <code>null</code> the complete
	 *            path of the audio file will be uses as stream name.
	 */
	...
	public void setInputStream(AudioInputStream inputStream, String streamName) {
		...
		if (format.getSampleSizeInBits() % 8 != 0) throw new Error(
				"StreamDataSource: bits per sample must be a multiple of 8.");
	...
	/**
	 * Reads and returns the next Data from the InputStream of StreamDataSource,
	 * return null if no data is read and end of file is reached.
	 * 
	...
	private DataEndSignal createDataEndSignal() {
		if (!(this instanceof ConcatAudioFileDataSource)) for (AudioFileProcessListener fileListener : fileListeners)
	...
	private Data readNextFrame() throws DataProcessingException {
		...
		try {
			...
			if (totalRead < bytesToRead) {
				totalRead = (totalRead % 2 == 0) ? totalRead + 2
						: totalRead + 3;
