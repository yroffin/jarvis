...
public class Microphone extends BaseDataProcessor {
	...
	/**
	 * The property that specifies whether or not the microphone will release
	 * the audio between utterances. On certain systems (Linux for one), closing
	 * and reopening the audio does not work too well. The default is false for
	...
	/**
	 * The property that specifies whether to keep the audio data of an
	 * utterance around until the next utterance is recorded.
	 */
	 ...
	 * Currently, the possible values are "average", which averages the samples
	 * from at each channel, or "selectChannel", which chooses audio only from
	 * that channel. If you choose "selectChannel", you should also specify
	 ...
	 * last Mixer supported by the AudioSystem), which appears to be what is
	 * often used for USB headsets, or an integer value which represents the
	 * index of the Mixer.Info that is returned by AudioSystem.getMixerInfo().
	 * To get the list of Mixer.Info objects, run the AudioTool application with
	 ...
	 *            reopening the audio does not work too well. The default is
	 *            false for Linux systems, true for others
	 * @param msecPerRead
	 *            the number of milliseconds of audio data to read each time
	 *            from the underlying Java Sound audio device.
	 * @param keepLastAudio
	 *            whether to keep the audio data of an utterance around until
	 *            the next utterance is recorded.
	 * @param stereoToMono
	 *            how to convert stereo audio to mono. Currently, the possible
	 *            values are "average", which averages the samples from at each
	 *            channel, or "selectChannel", which chooses audio only from
	 *            that channel. If you choose "selectChannel", you should also
	 *            specify which channel to use with the "selectChannel"
	 *            property.
	 * @param selectedChannel
	 *            the channel to use if the audio is stereo
	 * @param selectedMixerIndex
	 *            the mixer to use. The value can be "default," (which means let
	 *            the AudioSystem decide), "last," (which means select the last
	 *            Mixer supported by the AudioSystem), which appears to be what
	 *            is often used for USB headsets, or an integer value which
	 *            represents the index of the Mixer.Info that is returned by
	 *            AudioSystem.getMixerInfo(). To get the list of Mixer.Info
	 *            objects, run the AudioTool application with a command line
	 *            argument of "-dumpMixers".
	 */
	public Microphone(int sampleRate, int bitsPerSample, int channels,
			boolean bigEndian, boolean signed, boolean closeBetweenUtterances,
			int msecPerRead, boolean keepLastAudio, String stereoToMono,
		...
		this.desiredFormat = new AudioFormat(sampleRate, bitsPerSample,
	...
	public void newProperties(PropertySheet ps) throws PropertyException {
		...
		desiredFormat = new AudioFormat(sampleRate, sampleSizeInBits, channels,
				signed, bigEndian);
	...
	public void initialize() {
		...
		if (!AudioSystem.isLineSupported(info)) {
			logger.severe(desiredFormat + " not supported");
			AudioFormat nativeFormat = DataUtil.getNativeAudioFormat(
					desiredFormat, getSelectedMixer());
			if (nativeFormat == null) {
				...
				doConversion = AudioSystem.isConversionSupported(desiredFormat,
						nativeFormat);
				...
				if (doConversion) {
					...
					logger.info("Using native format: Cannot convert from "
							+ finalFormat.getSampleRate() + "Hz to "
							+ desiredFormat.getSampleRate() + "Hz");
				}
			}
	...
	private TargetDataLine getAudioLine() {
		...
		try {
			/*
			 * The finalFormat was decided in the initialize() method and is
			 * based upon the capabilities of the underlying audio system. The
			 * final format will have all the desired audio characteristics, but
			 * may have a sample rate that is higher than desired. The idea here
			 * is that we'll let the processors in the front end (e.g., the FFT)
			 * handle some form of downsampling for us.
			 */
			...
			/*
			 * We either get the audio from the AudioSystem (our default
			 * choice), or use a specific Mixer if the selectedMixerIndex
	...
	private boolean open() {
		...
		if (audioLine != null) {
			if (!audioLine.isOpen()) {
				...
				frameSizeInBytes = (audioStream.getFormat()
	...
	/**
	 * Returns the format of the audio recorded by this Microphone. Note that
	 * this might be different from the configured format.
	 * 
	...
	/**
	 * Starts recording audio. This method will return only when a START event
	 * is received, meaning that this Microphone has started capturing audio.
	 * 
	 ...
	 * been stopped and all data has been read from the audio line.
	 */
	...
	class RecordingThread extends Thread {
		 ...
		 * actually stopped, and all the data has been read from the audio line.
		 */
		public void stopRecording() {
			...
			// flush can not be called here because the audio-line might has
			// been set to null already by the mic-thread
			// audioLine.flush();
		}
		...
		public void run() {
			...
			if (keepDataReference) {
				currentUtterance = new Utterance("Microphone",
			...
			try {
				...
				if (closeBetweenUtterances) {
					/*
					 * Closing the audio stream *should* (we think) also close
					 * the audio line, but it doesn't appear to do this on the
					 * Mac. In addition, once the audio line is closed,
					 * re-opening it on the Mac causes some issues. The Java
					 * sound spec is also kind of ambiguous about whether a
					 * closed line can be re-opened. So...we'll go for the
					 * conservative route and never attempt to re-open a closed
					 * line.
					 */
					audioStream.close();
					audioLine.close();
			...
			long duration = (long) (((double) totalSamplesRead / (double) audioStream
					.getFormat().getSampleRate()) * 1000.0);
		...
		private Data readData(Utterance utterance) throws IOException {
			...
			int sampleSizeInBytes = audioStream.getFormat()
					.getSampleSizeInBits() / 8;
			...
			return (new DoubleData(samples, (int) audioStream.getFormat()
					.getSampleRate(), collectTime, firstSampleNumber));
		}
	}
	...
	/**
	 * Reads and returns the next Data object from this Microphone, return null
	 * if there is no more audio data. All audio data captured in-between
	...
	/**
	 * Returns true if there is more data in the Microphone. This happens either
	 * if the a DataEndSignal data was not taken from the buffer, or if the
	 * buffer in the Microphone is not yet empty.
	 * 
